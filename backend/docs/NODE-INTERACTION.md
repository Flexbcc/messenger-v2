...
## 9. Главный сервер (Discovery) и mesh push

Bootstrap-сценарий (ADR-0006): **главный** Discovery (`MAIN_IP` / `DISCOVERY_NODE_URL`) — точка регистрации всех нод.

```
Нода B (новая)  ──POST /registry/nodes──►  Discovery (главный)
Discovery       ──POST /internal/mesh/peer-joined──►  Нода A (существующая)
Нода A          ── merge peer B в локальный mesh cache ──► маршрутизация
```

| Шаг | Кто | Действие |
|-----|-----|----------|
| 1 | Новая нода | `POST /registry/nodes` + heartbeat каждые 60s |
| 2 | Discovery | При `trust_status=trusted` (legacy сразу или после admin approve) **push** соседям |
| 3 | Существующие ноды | `POST /internal/mesh/peer-joined` → локальный кеш peers |
| 4 | Fallback | `GET /registry/nodes` каждые 5 мин (`MESH_SYNC_INTERVAL_SECONDS`) |

Переменные: `MESH_NOTIFY_ENABLED`, `MESH_NOTIFY_SECRET`, `MESH_SYNC_INTERVAL_SECONDS`.

Локальное хранилище (storage-app / personal_pc): настройки `storage_ownership` в `profile_settings` → Home `GET /internal/users/{id}/storage-profile` → media-node (`HOME_NODE_URL`).
