...
class CatalogSync {
  ...
  static Future<void> syncNotifications() async {
    ...
    final sounds = await _store.getBool('notif_sounds', true);
    ...
    await _store.setBool(_key('notifications.enabled'), enabled && sounds);
    await _store.setString(_key('notifications.preview'), _previewToSpec(preview));
  ...
  static Future<void> syncHidden() async {
    ...
    await _store.setBool(_key('hidden.hide_from_search'), await store.excludeFromGlobalSearch());
    await _store.setBool(_key('hidden.hide_notifications'), await store.silenceNotifications());
    await _store.setBool(_key('hidden.hide_media'), await store.hideMediaFromGallery());
    await _store.setString(_key('hidden.open_method'), await store.openMethod());
