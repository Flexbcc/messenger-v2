...
class ChatPreferencesStore {
  ...
  Future<void> setLastRead(String conversationId, DateTime time) async {
    await _store.setInt('chat_last_read_ms_$conversationId', time.millisecondsSinceEpoch);
  }

  Future<bool> isMuted(String conversationId) async => _store.getBool('chat_mute_$conversationId', false);
  ...
  Future<int?> getDisappearingSeconds(String conversationId) async {
    final seconds = await _store.getInt('chat_disappear_sec_$conversationId', 0);
