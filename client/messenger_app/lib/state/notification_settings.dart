...
class NotificationSettings extends ChangeNotifier {
  ...
  Future<void> _load() async {
    ...
    enabledTypes = await _store.getStringList(SettingsCatalogBridge.catalogKey('notifications.types'));
    if (enabledTypes.isEmpty) {
      enabledTypes = const ['direct', 'mentions', 'replies', 'calls', 'security'];
    }

    dndEnabled = await _store.getBool(SettingsCatalogBridge.catalogKey('notifications.dnd_enabled'), false);
  ...
  bool shouldNotifyMessage({
    ...
    if (isHiddenChat) {
      return switch (hiddenChatPolicy) {
        'none' => false,
        'generic' => true,
        'normal' => _channelEnabled(hiddenChats == 'Выключено' ? 'Все сообщения' : hiddenChats, message, myDisplayName, myUserId, conversation.isGroup),
        _ => _channelEnabled(hiddenChats, message, myDisplayName, myUserId, conversation.isGroup),
      };
    }
  ...
  bool _typeAllowed(
    ...
    if (conversation.isGroup) {
      ...
      final mentionsMe = _mentionsMe(message.plaintext, myDisplayName, myUserId);
      if (mentionsMe && types.contains('mentions')) return true;
      if (message.replyToMessageId != null && types.contains('replies')) return true;
      return false;
    }
  ...
  bool _isInDndWindow({DateTime? now}) {
    ...
    if (dndSchedule.isEmpty) return true; // enabled with no ranges = always quiet
