...
void main() {
  group('extractBackupHomeUrls', () {
    test('collects alternate home_nodes excluding the primary', () {
      final routing = {
        'home_nodes': [
          {'node_id': 'h1', 'url': 'https://home-1.example', 'latency_ms': 12.0},
          {'node_id': 'h2', 'url': 'https://home-2.example', 'latency_ms': 40.0},
        ],
      };
    ...
    test('returns empty list when routing has no home_nodes', () {
      expect(extractBackupHomeUrls({'discovery_url': 'https://d'}, 'https://home'), isEmpty);
  ...
  group('NetworkBootstrap', () {
    test('fromJson derives backups from nested routing (invite redeem shape)', () {
      ...
      expect(bootstrap.allHomeUrls, containsAll(['https://home-1.example', 'https://home-2.example']));
    });

    test('fromJson derives backups from top-level home_nodes (GET /gateway/routing shape)', () {
      final bootstrap = NetworkBootstrap.fromJson({
        'cluster_id': 'default',
        'gateway_url': 'https://gw.example',
        'discovery_url': 'https://disc.example',
    ...
    test('round-trips backupHomeUrls through toJson/fromJson (persisted shape)', () {
    ...
    test('copyWith updates backups without touching primary home', () {
      ...
      final updated = original.copyWith(backupHomeUrls: ['https://home-2.example']);
