...
void main() {
  test('Two real users register, message each other, and only the recipient can decrypt', () async {
    ...
    await aliceCrypto.establishSessionFromBundle(bobUserId, fetchedBobBundle['bundle'] as Map<String, dynamic>);

    final conv = await aliceApi.createConversation(type: 'direct', participantUserIds: [bobUserId]);

    const plaintext = 'hello from the real client stack, never visible server-side';
    final ciphertext = await aliceCrypto.encrypt(bobUserId, utf8.encode(plaintext));
    ...
    final decrypted = await bobCrypto.decrypt(aliceUserId, receivedEnvelope['ciphertext'] as String);
