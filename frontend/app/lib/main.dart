...
import 'widgets/client_update_banner.dart';
...
class _MessengerAppState extends ConsumerState<MessengerApp>
    ...
    Future.microtask(() async {
      ...
      try {
        ...
        try {
          ...
          DebugLog.instance.error('boot', 'OS notification init failed', error);
        }
        try {
          await AppLockService.instance.init();
        } catch (error) {
          DebugLog.instance.error('boot', 'App lock init failed', error);
        }
        try {
          await bootstrapSettingsCatalog(
            ref.read,
          ).timeout(const Duration(seconds: 12));
        } catch (error) {
          DebugLog.instance.warn(
            'boot',
            'settings catalog bootstrap skipped',
            error,
          );
        }
        if (!mounted) return;
        await MessageFormatPrefs.reload();
        controller.notificationSettings = ref.read(
          notificationSettingsProvider,
        );
        await controller.boot().timeout(const Duration(seconds: 20));
      } catch (error, stackTrace) {
        DebugLog.instance.error('boot', 'application start failed', error);
  ...
  Widget build(BuildContext context) {
    ...
    final locale =
        ref.watch(appLocaleProvider).valueOrNull ?? const Locale('ru');
        ...
        child: SettingsAppearanceWrapper(
          child: AppLockOverlay(
            child: InAppNotificationHost(
              child: ClientUpdateBannerHost(
                child: Stack(
                  children: [
                    if (child != null) child,
                    if (controller.currentCall != null &&
                        controller.callUiMinimized)
