...
class SettingsCatalog {
  ...
  SettingsCatalog withoutSettings(Set<String> ids) => SettingsCatalog(
    ...
    sections: sections
        ...
        .where((section) => section.settings.isNotEmpty)
        .toList(growable: false),
  );
    ...
    switch (type) {
      ...
      case 'number':
        ...
        if (minimum != null && value < minimum!) {
          throw FormatException('Value is below minimum for $id');
        }
        if (maximum != null && value > maximum!) {
          throw FormatException('Value is above maximum for $id');
        }
        return;
      case 'single_select':
        if (value is! String || !options.contains(value)) {
          throw FormatException('Unknown option for $id');
        }
        return;
      case 'multi_select':
        if (value is! List) {
          throw const FormatException('Expected option list');
        }
        final selected = value.whereType<String>().toList(growable: false);
        if (selected.length != value.length ||
            selected.length != selected.toSet().length ||
            selected.any((option) => !options.contains(option))) {
          throw FormatException('Invalid option list for $id');
        }
        return;
      case 'text':
        if (value is! String) throw const FormatException('Expected text');
        final error = validateInput(value, number: false);
        if (error != null) throw FormatException('$id: $error');
        return;
      default:
        throw StateError('Setting type $type cannot be changed as a value');
    }
  }
}
