...
class _ChatScreenState extends ConsumerState<ChatScreen> {
  ...
  bool _sending = false;
  final List<({String text, String? replyToMessageId, String? replyPreview})>
  _pendingTextSends = [];
  ...
  DateTime? _scheduledMessageExpiry;
  ...
  void _scheduleMessageExpiry(AppController controller) {
    final expiry = controller.nextVisibleMessageExpiry(widget.conversation.id);
    if (expiry == _scheduledMessageExpiry) return;
    _messageExpiryTimer?.cancel();
    _scheduledMessageExpiry = expiry;
  ...
  Future<void> _sendText() async {
    ...
    _pendingTextSends.add((
      text: text,
      replyToMessageId: reply?.id,
      replyPreview: reply != null ? messageDisplayBody(reply) : null,
    ));
    _textController.clear();
    _draftAttachmentName = null;
    setState(() => _replyTo = null);
    ...
    while (_pendingTextSends.isNotEmpty) {
      final pending = _pendingTextSends.removeAt(0);
      ref
          .read(appControllerProvider)
          .touchSecretSession(widget.conversation.id);
      try {
        await ref
            .read(appControllerProvider)
            .sendText(
  ...
  Widget build(BuildContext context) {
    ...
    _scheduleMessageExpiry(controller);
                    ...
                    child: Row(
                      ...
                      children: [
                        ...
                        IconButton(
                          ...
                          onPressed: !_canSend ? null : _sendText,
                        ),
                      ],
                    ),
                  ),
