...
class _ContactProfileScreenState extends ConsumerState<ContactProfileScreen> {
  ...
  Future<void> _loadCallPrefs() async {
    ...
    final showPhone = await runtime.canShowPhone(
      widget.userId,
      isContact: true,
    );
    final showEmail = await runtime.canShowEmail(
      widget.userId,
      isContact: true,
    );
    final showAvatar = await runtime.canShowAvatar(
      widget.userId,
      isContact: true,
    );
    final qrOnly = await runtime.qrOnlyMode();
    final blocked = await runtime.isBlocked(widget.userId);
    if (!mounted) return;
    setState(() {
  ...
  Future<void> _toggleBlock() async {
    ...
    if (!_blocked) {
      final confirmed = await showDialog<bool>(
        ...
        builder: (context) => AlertDialog(
          ...
          content: const Text(
            'Новые сообщения и звонки от этого контакта не будут приниматься. '
            'История переписки останется на устройстве.',
          ),
          actions: [
            TextButton(
    ...
    try {
      if (_blocked) {
        await SettingsRuntime.instance.unblockUser(widget.userId);
      } else {
        await SettingsRuntime.instance.blockUser(widget.userId);
      }
      if (!mounted) return;
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            _blocked ? 'Контакт заблокирован' : 'Контакт разблокирован',
          ),
        ),
      );
    } finally {
      if (mounted) setState(() => _updatingBlock = false);
    }
  }
  ...
  Future<void> _setTrust(TrustLevel level) async {
    ...
    await ref
        ...
        .setContactTrustLevel(widget.userId, level);
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Уровень доверия: ${level.label}')),
      );
    }
  }
  ...
  Future<void> _openChat() async {
    try {
      final conv = await ref
          .read(appControllerProvider)
          .openOrCreateDirectChat(widget.userId);
      if (!mounted) return;
      await Navigator.of(
        context,
  ...
  bool _trustLevelAllowed(TrustLevel level) {
    ...
    return _allowedTrustLevels.contains(level.name);
  }
  ...
  Widget build(BuildContext context) {
    ...
    final name =
        controller.knownDisplayNames[widget.userId] ?? widget.displayName;
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            ...
            children: [
              AppQuickAction(
                ...
                label: _blocked ? 'Заблокирован' : 'Написать',
                onTap: _blocked ? null : _openChat,
              ),
              AppQuickAction(
                icon: Icons.call_outlined,
                label: 'Аудио',
                onTap: _blocked || _calling || controller.currentCall != null
                    ? null
                    : () => _call(CallKind.audio),
              ),
                ...
                children: [
                  for (var i = 0; i < TrustLevel.values.length; i++) ...[
                    if (_trustLevelAllowed(TrustLevel.values[i])) ...[
                      ...
                      ListTile(
                        leading: Icon(
                          _trustIcon(TrustLevel.values[i]),
                          color: _trustColor(TrustLevel.values[i], colors),
                        ),
                        title: Text(
                          TrustLevel.values[i].label,
                          style: text.subtitle,
                        ),
                        subtitle: Text(
                          TrustLevel.values[i].description,
                          style: text.caption,
                        ),
          ...
          AppButton(
            ...
            onPressed: _updatingBlock ? null : _toggleBlock,
            expanded: false,
          ),
          const SizedBox(height: AppSpacing.xl),
          AppCard(
                ...
                IconButton(
                  ...
                  onPressed: () async {
                    ...
                    messenger.showSnackBar(
                      const SnackBar(
                        content: Text('Карточка контакта скопирована'),
                      ),
                    );
                  },
                ),
