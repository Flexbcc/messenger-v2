...
class DeviceCrypto {
  ...
  Future<Map<String, dynamic>?> decryptJson(String packed) async {
    ...
    debugPrint(
      'DeviceCrypto.decryptJson failed: SecretBoxAuthenticationError (no matching key)',
    );
    return null;
  }
  ...
  Future<void> wipeLocalKey() async {
    ...
    try {
      await _legacyMacStorage.delete(key: _keyName);
    } catch (_) {}
    try {
      await _plainMacStorage.delete(key: _keyName);
    } catch (_) {}
    final prefs = await SharedPreferences.getInstance();
    await prefs.remove('secure_fallback::$_keyName');
    invalidateCache();
  }
