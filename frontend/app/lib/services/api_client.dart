...
class ApiClient {
  ...
  Uri _discoveryUri(String path) =>
      Uri.parse('${AppConfig.discoveryNodeUrl}$path');
  ...
  Future<void> registerWebPush(String subscription) async {
    final body = jsonEncode({'subscription': subscription});
    final resp = await http.put(
      _homeUri('/users/me/push'),
      headers: _headers,
      body: body,
    );
    _decodeOrThrow(resp);
  ...
  Future<Map<String, dynamic>> pollDeviceLink({
    ...
    final resp = await _postJson(_homeUri('/auth/device-links/$linkId/poll'), {
      'secret': secret,
    });
    return _decodeOrThrow(resp) as Map<String, dynamic>;
  }
  ...
  Future<void> approveDeviceLink({
    ...
    final resp = await _postJson(
      _homeUri('/auth/device-links/$linkId/approve'),
      {'secret': secret},
    );
    _decodeOrThrow(resp);
  }
  ...
  Future<Map<String, dynamic>> inspectDeviceLink({
    ...
    final resp = await _postJson(
      _homeUri('/auth/device-links/$linkId/inspect'),
      {'secret': secret},
    );
    return _decodeOrThrow(resp) as Map<String, dynamic>;
  }
  ...
  Future<void> updatePresencePolicy({
    ...
    final resp = await http.put(
      _homeUri('/users/me/presence-policy'),
      headers: _headers,
      body: encoded,
    );
    _decodeOrThrow(resp);
  ...
  Future<Map<String, dynamic>> getPresence(String userId) async {
    final resp = await _get(_homeUri('/users/$userId/presence'));
    return _decodeOrThrow(resp) as Map<String, dynamic>;
  }
  ...
  Future<Map<String, dynamic>> searchUserByLogin(String login) async {
    final resp = await _get(
      _discoveryUri(
        '/registry/users/search?login=${Uri.encodeQueryComponent(login)}',
      ),
    );
    return _decodeOrThrow(resp) as Map<String, dynamic>;
  }
  ...
  Future<void> ackMessage(String conversationId, String packetId) async {
    final resp = await _post(
      _homeUri('/conversations/$conversationId/messages/$packetId/ack'),
    );
    _decodeOrThrow(resp);
  }
  ...
  Future<List<dynamic>> findNodes({required String capability}) async {
    final resp = await _get(
      _discoveryUri(
        '/registry/nodes',
      ).replace(queryParameters: {'capability': capability}),
    );
