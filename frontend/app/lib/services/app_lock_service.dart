...
class AppLockService extends ChangeNotifier {
  ...
  Future<void> init() async {
    ...
    // Account scope changes must not carry lock state or timers from the
    // previous account into the next session.
    // A cold start is a new security boundary.  In particular this covers a
    // browser reload where Flutter does not always emit a paused/hidden event
    // before the page is destroyed.
    isLocked = enabled && await PinSecurity.isRealPinConfigured();
    _armed = false;
    _armedAt = null;
  ...
  /// Immediately hides application data behind the primary PIN.
  ///
  /// Decoy/panic exits use this even when ordinary background locking is
  /// disabled: returning from a decoy view must never expose the real account.
  Future<void> lockNow({bool force = false}) async {
    if (!force && !enabled) return;
    if (!await PinSecurity.isRealPinConfigured()) return;
    isLocked = true;
    _armed = false;
    _armedAt = null;
    _forceOnResume = false;
