import 'dart:async';
import 'dart:convert';
import 'dart:io' show Platform;

import 'package:flutter/foundation.dart';
import 'package:http/http.dart' as http;
import 'package:url_launcher/url_launcher.dart';

import '../config.dart';
import '../models/client_release_manifest.dart';
import 'pwa_update_bridge.dart';

/// Checks the release manifest and the independently published web build id.
class ClientUpdateService extends ChangeNotifier {
  ClientUpdateService._();
  static final instance = ClientUpdateService._();

  bool _started = false;
  bool _checking = false;
  Timer? _timer;

  static const _manifestMaxBytes = 512 * 1024;
  static const _buildMarkerMaxBytes = 64 * 1024;

  bool pwaReloadReady = false;
  bool manifestUpdateAvailable = false;
  String? releaseNotes;
  String? downloadUrl;
  String? remoteVersion;
  int? remoteBuild;
  String updateKind = 'download';
  String? lastError;

  bool get hasUpdate => pwaReloadReady || manifestUpdateAvailable;

  String? get bannerMessage {
    if (pwaReloadReady) {
      return 'Доступна новая версия веб-клиента';
    }
    if (manifestUpdateAvailable && remoteVersion != null) {
      return 'Доступна версия $remoteVersion (${AppInfo.channel})';
    }
    return null;
  }

  void start() {
    if (_started) return;
    _started = true;

    unawaited(checkForUpdates());
    _timer = Timer.periodic(
      kIsWeb ? const Duration(minutes: 5) : const Duration(hours: 6),
      (_) => checkForUpdates(),
    );
  }

  void stop() {
    _timer?.cancel();
    _timer = null;
    _started = false;
  }

  Future<void> checkForUpdates() async {
    if (_checking) return;
    _checking = true;
    try {
      lastError = null;
      if (kIsWeb && PwaUpdateBridge.instance.isSupported) {
        await _checkWebBuild();
      }
      final uri = Uri.parse(
        '${AppConfig.gatewayNodeUrl}/releases/clients/manifest.json'
        '?_=${DateTime.now().millisecondsSinceEpoch}',
      );
      final res = await _getBoundedText(
        uri,
        maxBytes: _manifestMaxBytes,
        timeout: const Duration(seconds: 12),
      );
      if (res.statusCode != 200) {
        lastError = 'manifest ${res.statusCode}';
        notifyListeners();
        return;
      }
      final decoded = jsonDecode(res.body);
      if (decoded is! Map<String, dynamic>) {
        throw const FormatException('Release manifest must be an object');
      }
      final manifest = ClientReleaseManifest.fromJson(decoded);
      if (manifest.channel != AppInfo.channel ||
          manifest.messenger.channel != AppInfo.channel) {
        throw const FormatException(
          'Release manifest channel does not match this build',
        );
      }
      final platform = _currentPlatformKey();
      if (platform == null) {
        _clearManifestUpdate();
        lastError = 'updates are unsupported on this platform';
        notifyListeners();
        return;
      }
      final plat = manifest.messenger.forPlatform(platform);
      if (plat == null || !plat.available) {
        _clearManifestUpdate();
        notifyListeners();
        return;
      }

      remoteVersion = plat.version;
      remoteBuild = plat.build;
      releaseNotes = manifest.messenger.releaseNotes;
      updateKind = plat.updateKind;
      downloadUrl = _validatedDownloadUrl(plat.downloadUrl);
      if (updateKind == 'reload' && !PwaUpdateBridge.instance.isSupported) {
        _clearManifestUpdate();
        lastError = 'reload updates are unsupported on this target';
        notifyListeners();
        return;
      }
      if (updateKind != 'reload' && downloadUrl == null) {
        throw const FormatException(
          'Release manifest contains an unsafe download URL',
        );
      }

      manifestUpdateAvailable = isRemoteNewer(
        localVersion: AppInfo.version,
        localBuild: int.tryParse(AppInfo.buildNumber) ?? 0,
        remoteVersion: plat.version,
        remoteBuild: plat.build,
      );
      notifyListeners();
    } catch (e) {
      lastError = e.toString();
      notifyListeners();
    } finally {
      _checking = false;
    }
  }

  void _clearManifestUpdate() {
    manifestUpdateAvailable = false;
    releaseNotes = null;
    downloadUrl = null;
    remoteVersion = null;
    remoteBuild = null;
    updateKind = 'download';
  }

  Future<void> _checkWebBuild() async {
    // Old builds (before APP_BUILD_ID was introduced) cannot be compared. They
    // will be replaced once manually, after which all deployments are detected.
    if (AppInfo.buildId.isEmpty) return;
    try {
      final uri = Uri.base.resolve(
        'deploy-version.json?_=${DateTime.now().millisecondsSinceEpoch}',
      );
      final res = await _getBoundedText(
        uri,
        maxBytes: _buildMarkerMaxBytes,
        timeout: const Duration(seconds: 8),
      );
      if (res.statusCode != 200) return;
      final decoded = jsonDecode(res.body);
      if (decoded is! Map<String, dynamic>) return;
      final body = decoded;
      final remoteBuildId = body['build_id']?.toString() ?? '';
      if (remoteBuildId.isNotEmpty && remoteBuildId != AppInfo.buildId) {
        pwaReloadReady = true;
        notifyListeners();
      }
    } catch (_) {
      // The regular release-manifest check below must still run when the small
      // static version file is temporarily unavailable.
    }
  }

  Future<void> applyUpdate() async {
    if (pwaReloadReady || (kIsWeb && updateKind == 'reload')) {
      if (!PwaUpdateBridge.instance.isSupported) {
        throw UnsupportedError(
          'Application reload is not supported on this target',
        );
      }
      await PwaUpdateBridge.instance.applyReload();
      return;
    }
    final url = downloadUrl;
    if (url != null && url.isNotEmpty) {
      final uri = Uri.parse(url);
      if (await canLaunchUrl(uri)) {
        final launched = await launchUrl(
          uri,
          mode: LaunchMode.externalApplication,
        );
        if (!launched) {
          throw StateError('The update URL could not be opened');
        }
        return;
      }
      throw StateError('The update URL is not supported on this device');
    }
    throw StateError('No applicable update is available');
  }

  void dismissForSession() {
    pwaReloadReady = false;
    manifestUpdateAvailable = false;
    notifyListeners();
  }

  String? _validatedDownloadUrl(String? raw) {
    if (raw == null || raw.isEmpty || raw.length > 2048) return null;
    final uri = Uri.tryParse(raw);
    if (uri == null ||
        !uri.hasAuthority ||
        uri.host.isEmpty ||
        uri.userInfo.isNotEmpty ||
        uri.fragment.isNotEmpty) {
      return null;
    }
    if (uri.scheme == 'https') return uri.toString();
    final loopback =
        uri.host == 'localhost' || uri.host == '127.0.0.1' || uri.host == '::1';
    return uri.scheme == 'http' && loopback ? uri.toString() : null;
  }

  Future<({int statusCode, String body})> _getBoundedText(
    Uri uri, {
    required int maxBytes,
    required Duration timeout,
  }) async {
    final client = http.Client();
    try {
      final request = http.Request('GET', uri)
        ..headers['Cache-Control'] = 'no-cache'
        ..followRedirects = false
        ..maxRedirects = 0;
      final response = await client.send(request).timeout(timeout);
      final declaredLength = response.contentLength;
      if (declaredLength != null && declaredLength > maxBytes) {
        throw const FormatException('Update response is too large');
      }
      final bytes = <int>[];
      await for (final chunk in response.stream.timeout(timeout)) {
        if (bytes.length + chunk.length > maxBytes) {
          throw const FormatException('Update response is too large');
        }
        bytes.addAll(chunk);
      }
      return (
        statusCode: response.statusCode,
        body: utf8.decode(bytes, allowMalformed: false),
      );
    } finally {
      client.close();
    }
  }

  String? _currentPlatformKey() {
    if (kIsWeb) return 'web';
    try {
      if (Platform.isMacOS) return 'macos';
      if (Platform.isWindows) return 'windows';
      if (Platform.isLinux) return 'linux';
      if (Platform.isAndroid) return 'android';
      if (Platform.isIOS) return 'ios';
    } catch (_) {
      return null;
    }
    return null;
  }
}
