...
class LoginApprovalService {
  ...
  Future<List<LoginApprovalRequest>> pendingRequests({
    required Iterable<DeviceInfo> devices,
    required bool Function(String deviceId) isTrusted,
  }) async {
    final pending = <LoginApprovalRequest>[];
    for (final device in devices) {
      if (device.isCurrent || isTrusted(device.id) || !isRecentLogin(device)) {
        continue;
      }
      if (await isDismissed(device.id)) continue;
      pending.add(LoginApprovalRequest.fromDevice(device));
    }
    return pending;
  }
