...
class MessageCacheStore {
  ...
  Future<void> _webSaveConversation(
    ...
    if (!await prefs.setString(_webConvKey(userId, conversationId), packed)) {
      throw StateError('Unable to persist encrypted message cache');
    }
  }
  ...
  Future<List<ChatMessage>> _webLoadConversation(
    ...
    final items = rawItems.length > _webMaxPerConversation
        ? rawItems.skip(rawItems.length - _webMaxPerConversation)
        : rawItems;
    final messages = <ChatMessage>[];
    for (final item in items) {
      if (item is! Map<String, dynamic>) continue;
      try {
        messages.add(_messageFromJson(item));
      } catch (_) {
        // A malformed cache entry must not prevent the account from opening.
      }
    }
    return messages;
  }
  ...
  Future<List<ChatMessage>> loadConversation(
    ...
    if (deadIds.isNotEmpty) {
      ...
      DebugLog.instance.warn(
        'cache',
        'purged ${deadIds.length} unreadable message rows',
      );
    }
    return out;
  }
    ...
    if (kIsWeb) {
      ...
      for (final key in prefs.getKeys()) {
        if (key.startsWith(prefix) && !await prefs.remove(key)) {
          throw StateError('Unable to clear encrypted message cache');
        }
      }
      return;
    }
  ...
  Future<void> clearConversation(String userId, String conversationId) async {
    if (kIsWeb) {
      final prefs = await SharedPreferences.getInstance();
      final key = _webConvKey(userId, conversationId);
      if (prefs.containsKey(key) && !await prefs.remove(key)) {
        throw StateError('Unable to clear encrypted conversation cache');
      }
      return;
    }
    final db = await _database();
