...
class PersistentMediaStore {
  ...
  Future<Uint8List?> getCiphertext(String userId, String mediaId) async {
    ...
    final decoded = bytes is Uint8List
        ? bytes
        : Uint8List.fromList((bytes as List).cast<int>());
    validateMediaStorageInput(userId, mediaId, bytes: decoded);
    return decoded;
  }
  ...
  Future<void> importUser(String userId, Map<String, dynamic> values) async {
    validateMediaStorageUser(userId);
    for (final entry in decodeMediaBackup(values)) {
      await putCiphertext(userId, entry.mediaId, entry.bytes);
    }
  }
