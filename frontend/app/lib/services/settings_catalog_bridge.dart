...
class SettingsCatalogBridge {
  SettingsCatalogBridge({LocalSettingsStore? store})
    ...
    switch (id) {
      ...
      case 'notifications.preview':
        await _store.setString(
          'notif_preview',
          _previewToLegacy(value?.toString()),
        );
        break;
      ...
      case 'notifications.dnd_enabled':
        await _store.setBool(
          catalogKey('notifications.dnd_enabled'),
          value == true,
        );
        break;
      case 'notifications.hidden_chat_policy':
        ...
        await _store.setString(
          catalogKey('notifications.hidden_chat_policy'),
          policy,
        );
      ...
      case 'security.autolock':
        await PrivacyPreferencesStore().setAutoLockSeconds(
          _autolockToSeconds(value?.toString()),
        );
        break;
      ...
      case 'hidden.open_method':
        ...
        await HiddenChatsStore.instance.setGestureEntryEnabled(
          method == 'gesture',
        );
        break;
      case 'hidden.hide_from_search':
        await HiddenChatsStore.instance.setExcludeFromGlobalSearch(
          value == true,
        );
        break;
      ...
      case 'contacts.trusted_enabled':
        await _store.setBool(
          catalogKey('contacts.trusted_enabled'),
          value == true,
        );
        break;
  ...
  Future<void> _syncAllMediaAutoload() async {
    final wifiKinds = await _store.getStringList(
      catalogKey('media.autoload_wifi'),
    );
    final mobileKinds = await _store.getStringList(
      catalogKey('media.autoload_mobile'),
    );
...
class CatalogSettingsReader {
  CatalogSettingsReader({LocalSettingsStore? store})
    : _store = store ?? LocalSettingsStore();
