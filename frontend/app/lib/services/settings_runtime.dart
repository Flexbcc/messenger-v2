...
class SettingsRuntime {
  SettingsRuntime({CatalogSettingsReader? reader, CatalogListStore? lists})
  ...
  Future<bool> usernameEnabled() =>
      _reader.getBool('profile.username_enabled', false);
  ...
  Future<String> timeFormat() =>
      _reader.getString('profile.time_format', '24h');
  Future<String> dateFormat() =>
      _reader.getString('profile.date_format', 'DD.MM.YYYY');

  // ── Identity ───────────────────────────────────────────────────────────────

  Future<bool> phoneEnabled() =>
      _reader.getBool('identity.phone_enabled', false);
  Future<String> phone() => _reader.getString('identity.phone', '');
  Future<bool> emailEnabled() =>
      _reader.getBool('identity.email_enabled', false);
  ...
  Future<bool> typingEnabled() =>
      _reader.getBool('privacy.typing_status', true);
  Future<bool> onlineStatusEnabled() =>
      _reader.getBool('privacy.online_status', true);
  Future<bool> invisibleMode() =>
      _reader.getBool('privacy.invisible_mode', false);
  Future<String> lastSeenPolicy() =>
      _reader.getString('privacy.last_seen', 'contacts');
  Future<bool> usernameSearchAllowed() =>
      _reader.getBool('privacy.username_search', true);
  Future<String> phoneSearchPolicy() =>
      _reader.getString('privacy.phone_search', 'nobody');
  Future<bool> readReceiptsVisible() =>
      _reader.getBool('privacy.read_receipts', true);
  ...
  Future<bool> groupInviteAllowed(
    String inviterId, {
    required bool isContact,
  }) async {
  ...
  Future<bool> canShowOnlineStatus(
    String viewerUserId, {
    required bool isContact,
  }) async {
  ...
  Future<bool> incomingMessagesAllowed(
    String senderId, {
    required bool isContact,
  }) async {
    final policy = await _reader.getString(
  ...
  Future<List<String>> fakeProfileChats() =>
      _lists.load('security.fake_profile_chats');

  Future<bool> distressSignalEnabled() =>
      _reader.getBool('security.distress_signal', false);

  Future<List<String>> distressContacts() =>
      _lists.load('security.distress_contacts');

  Future<List<String>> requirePinForCritical() async {
    final saved = await _reader.getStringList(
  ...
  /// `pin` | `gesture` | `secret_command`
  Future<String> hiddenOpenMethod() =>
      _reader.getString('hidden.open_method', 'pin');

  Future<bool> hiddenHideFromSearch() =>
      _reader.getBool('hidden.hide_from_search', true);
  Future<bool> hiddenHideNotifications() =>
      _reader.getBool('hidden.hide_notifications', true);
  ...
  // PWA has no phone-book integration or server-side phone-hash directory.
  // Kept as a false fallback for older screens during catalog migration.
  Future<bool> contactsImportEnabled() async => false;
  Future<bool> contactsHashLookup() async => false;

  Future<bool> contactsTrustedEnabled() =>
      _reader.getBool('contacts.trusted_enabled', false);
  ...
  Future<List<String>> contactsAllowedTrustLevels() async {
    ...
    if (saved.isNotEmpty) return saved;
    return const ['unknown', 'normal', 'trusted', 'highTrust'];
  }

  Future<bool> keyChangeWarning() =>
      _reader.getBool('contacts.key_change_warning', true);
  Future<bool> blockOnKeyChange() =>
      _reader.getBool('contacts.block_on_key_change', false);
  ...
  Future<bool> confirmDelete() =>
      _reader.getBool('messages.confirm_delete', true);
  Future<String> linkPreviewMode() =>
      _reader.getString('messages.link_preview', 'local_only');
  Future<bool> linkPreviewEnabled() async {
    final mode = await linkPreviewMode();
    return mode != 'off';
  }

  Future<bool> confirmLargeFiles() =>
      _reader.getBool('messages.confirm_large_files', true);
  Future<int> largeFileConfirmMb() =>
      _reader.getInt('messages.large_file_mb', 100);
  ...
  Future<bool> autoDeleteEnabled() =>
      _reader.getBool('messages.auto_delete_enabled', false);
  Future<String> autoDeleteTtl() =>
      _reader.getString('messages.auto_delete_ttl', '7d');
  ...
  Future<String> autoCleanupAfter() =>
      _reader.getString('media.auto_cleanup_after', '30d');
  ...
  Future<bool> nodeCustomEnabled() =>
      _reader.getBool('node.custom_enabled', false);
  Future<String> nodeCustomAddress() =>
      _reader.getString('node.custom_address', '');
  Future<bool> nodeAllowFallback() =>
      _reader.getBool('node.allow_fallback', true);
  Future<bool> nodeAllowRelays() => _reader.getBool('node.allow_relays', true);
  Future<bool> nodeAllowServiceNodes() =>
      _reader.getBool('node.allow_service_nodes', true);
  ...
  Future<bool> nodeProxyEnabled() =>
      _reader.getBool('node.proxy_enabled', false);
  Future<String> nodeProxyType() =>
      _reader.getString('node.proxy_type', 'socks5');
  Future<String> nodeProxyAddress() =>
      _reader.getString('node.proxy_address', '');
  ...
  Future<String> syncHistoryDepth() =>
      _reader.getString('sync.history_depth', '30d');
  ...
  Future<String> backupSchedule() =>
      _reader.getString('backup.schedule', 'weekly');
  Future<bool> backupEncryption() => _reader.getBool('backup.encryption', true);
  Future<String> backupPassword() async =>
      await SecurePrefs.instance.read('backup_password_v1') ?? '';
  ...
  Future<List<String>> storageMessageNodes() =>
      _lists.load('storage.message_nodes');
  Future<int> storageReplicationFactor() =>
      _reader.getInt('storage.replication_factor', 1);
  Future<String> storageS3Endpoint() =>
      _reader.getString('storage.s3_endpoint', '');
  Future<String> storageS3Bucket() =>
      _reader.getString('storage.s3_bucket', '');
  Future<String> storageS3AccessKey() =>
      _reader.getString('storage.s3_access_key', '');
  Future<String> storageS3SecretKey() =>
      _reader.getString('storage.s3_secret_key', '');
  Future<bool> storageMediaTtlEnabled() =>
      _reader.getBool('storage.media_ttl_enabled', false);
  Future<String> storageMediaTtl() =>
      _reader.getString('storage.media_ttl', '30d');
  Future<String> storageKeyLocation() =>
      _reader.getString('storage.key_location', 'device');
  Future<String> storageBackupLocation() async => 'local_encrypted_file';
  Future<List<String>> storageAccessDevices() async => const [];
  Future<List<String>> storageAccessNodes() =>
      _lists.load('storage.access_nodes');
  ...
  Future<String> callsQuality() =>
      _reader.getString('calls.quality', 'balanced');
  Future<bool> callsNoiseSuppression() =>
      _reader.getBool('calls.noise_suppression', true);
  Future<bool> callsEchoCancellation() =>
      _reader.getBool('calls.echo_cancellation', true);
  ...
  Future<bool> animationsEnabled() =>
      _reader.getBool('appearance.animations', true);
  ...
  Future<bool> developerEnabled() =>
      _reader.getBool('developer.enabled', false);

  Future<bool> keepLocalDataOnLogout() =>
      _reader.getBool('data.keep_local_on_logout', true);
