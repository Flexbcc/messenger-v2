...
class SurbDeliveryStore {
  ...
  Future<List<SurbReplyBlock>> _read(String key) async {
    ...
    for (final item in items.take(_maxPerPeer)) {
      try {
  ...
  Future<void> _write(String key, List<SurbReplyBlock> blocks) async {
    ...
    if (blocks.isEmpty) {
      if (prefs.containsKey(key) && !await prefs.remove(key)) {
        throw StateError('Unable to remove encrypted SURB state');
      }
      return;
    }
    ...
    if (!await prefs.setString(key, packed)) {
      throw StateError('Unable to persist encrypted SURB state');
    }
  }
