...
class AppController extends ChangeNotifier {
  ...
  final Map<String, bool> _contactOnline = {};
  final Map<String, DateTime?> _contactLastSeen = {};
      ...
      if (existing != null) {
        try {
          ...
          DebugLog.instance.warn(
            'auth',
            'stale session ${e.statusCode}, clearing local login',
          );
          ...
          DebugLog.instance.error(
            'auth',
            'session restore failed, clearing local login: $e',
          );
          await _sessionStore.clear();
          session = null;
          _api.accessToken = null;
  ...
  Future<void> toggleMessagePinned(String messageId) async {
    ...
    await MessageLocalActionsStore.instance.setPinned(
      userId,
      messageId,
      !pinned,
    );
    ...
    try {
      ...
      if (authKeyPair != null) {
        try {
          ...
          DebugLog.instance.warn(
            'routing',
            'session recovery on new home failed: $e',
          );
        }
      }
      if (!recovered) {
  ...
  Future<void> loginWithLocalKey() async {
    ...
    final signature = await authKeyPair!.signBase64(base64Decode(nonce));
    final result = await _api.verify(
      deviceId: remembered.deviceId,
      nonce: nonce,
      signature: signature,
    );
    await _finishLogin(result, remembered.displayName);
  }
  ...
  Future<bool> pollDeviceLink(String linkId, String secret) async {
    final result = await _api.pollDeviceLink(linkId: linkId, secret: secret);
  ...
  Map<String, String> parseDeviceLinkPayload(String rawPayload) {
    ...
    final linkId = value['id'] as String?;
    final secret = value['secret'] as String?;
    if (linkId == null || secret == null || linkId.isEmpty || secret.isEmpty) {
  ...
  void _connectRealtime() {
    if (session == null) return;
    // Multi-device v0 (0405): every (re)connect — not just app resume — may
    // have missed a mirrored own-send or a message that arrived during a
    // network blip, so catch up on each one, not only the explicit resume path.
  ...
  Future<void> _catchUpMessagesAfterResume() async {
    ...
    for (final conversationId in ids) {
      try {
        ...
        DebugLog.instance.error(
          'sync',
          'catch-up failed for $conversationId: $e',
        );
      }
    }
  }
  ...
  Future<void> _catchUpConversation(String conversationId) async {
    ...
    final originalLocalIds = local.map((m) => m.id).toSet();
    ...
    for (final row in raw) {
      ...
      if (_absorbOutgoingEcho(local, m)) {
        // _absorbOutgoingEcho replaced a same-index entry in `local` in
        // place (pending/dup echo) — drop that entry's now-stale old id
        // from the merge map so it doesn't linger as a duplicate bubble
        // next to the confirmed message under m.id.
        final localIds = local.map((x) => x.id).toSet();
  ...
  Future<void> _onRealtimeEvent(Map<String, dynamic> event) async {
    ...
    if (msg.senderUserId != session?.userId) {
      ...
      final weMessaged =
          prior?.any((m) => m.senderUserId == session?.userId) ?? false;
      if (!weMessaged &&
        ...
        DebugLog.instance.info(
          'delivery',
          'incoming message blocked by privacy.incoming_messages',
        );
        return;
      }
    }
  ...
  Future<void> _processIncomingDistribution(ChatMessage msg) async {
    ...
    await _cryptoDecryptQueue.run('direct:${msg.senderUserId}', () async {
      try {
        final plaintextBytes = await crypto!.decrypt(
          msg.senderUserId,
          msg.ciphertext,
        );
        final payload =
  ...
  Future<void> _handleIncomingCallSignal(ChatMessage msg) async {
    ...
    switch (signal.type) {
      case CallSignalType.offer:
        ...
        callUiMinimized =
            false; // ensure incoming call screen shows even if previous call was minimized
  ...
  Future<void> refreshConversations() async {
    ...
    for (final c in conversations) {
      ...
      DebugLog.instance.info(
        'chat',
        '${c.id.substring(0, 8)}… peer=$peer reachable=$ok',
      );
    }
  ...
  void _sortConversations() {
    conversations.sort(
      (a, b) => _conversationActivity(b).compareTo(_conversationActivity(a)),
    );
  }
  ...
  List<Conversation> get sortedConversations {
    final copy = conversations
        .where(
          (c) =>
              !_hiddenConversationIds.contains(c.id) &&
              !_secretHiddenConversationIds.contains(c.id),
        )
        .toList();
    copy.sort(
      (a, b) => _conversationActivity(b).compareTo(_conversationActivity(a)),
  ...
  List<Conversation> conversationsMatchingSearch(String query) {
    ...
    return pool
        .where((c) => conversationTitle(c).toLowerCase().contains(q))
        .toList();
  }
  ...
  Future<void> _loadFavoritesPreferences() async {
    favoritesChatEnabled = await FavoritesPreferencesStore.instance
        .isChatEnabled();
  }
  ...
  List<Conversation> get secretHiddenConversations {
    final copy = conversations
        .where((c) => _secretHiddenConversationIds.contains(c.id))
        .toList();
  ...
  Future<void> _processDueReminders() async {
    ...
    final due = (await MessageReminderStore.instance.loadAll())
        .where((r) => !r.remindAt.isAfter(now))
        .toList();
  ...
  Future<void> _loadPrivacyPolicies() async {
    ...
    if (session != null) {
      try {
        ...
        DebugLog.instance.warn('presence', 'policy update failed: $e');
      }
    }
  }
  ...
  bool isSecretSessionActive(String conversationId) =>
      _secretSessionActive.contains(conversationId);
  ...
  void _resetSecretSessionTimer(String conversationId) {
    ...
    SecretChatPreferencesStore.instance.sessionTimeoutMinutes().then((minutes) {
      ...
      _secretSessionTimers[conversationId] = Timer(
  ...
  Future<void> relaySecuritySignal({
    ...
    final ok = await SecuritySignalClient(
      _api,
    ).relay(event: event, targets: trusted);
    if (ok) {
      await DuressAuditService.instance.recordOutbound(
        code: event,
        channel: 'relay',
  ...
  List<ChatMessage> visibleMessagesFor(String conversationId) {
    ...
    var visible = all.where(
      (m) => !_locallyHiddenMessageIds.contains(m.id) && !_isNonChatEnvelope(m),
    );
    if (!secretOn) {
    ...
    if (secretSeconds != null) {
      ...
      visible = visible.where(
        (m) => !m.isSecret || m.createdAt.isAfter(cutoff),
      );
    }
  ...
  List<ChatMessage> imageMessagesFor(String conversationId) {
    return visibleMessagesFor(
      conversationId,
    ).where((m) => m.contentType == 'image' && !m.decryptFailed).toList();
  }
  ...
  Future<void> loadHistory(String conversationId) async {
    ...
    final plaintextIndex = _plaintextIndexFor(conversationId, [
      ...diskCached,
      ...prior,
    ]);
    ...
    final mergedById = <String, ChatMessage>{for (final m in prior) m.id: m};
  ...
  Future<void> _ensureSessionWith(String otherUserId) async {
    ...
    try {
      await crypto!.establishSessionFromBundle(
        otherUserId,
        bundleResp['bundle'] as Map<String, dynamic>,
      );
    } catch (e) {
  ...
  bool _isUntrustedIdentityError(Object e) {
    final s = e.toString();
    return s.contains('UntrustedIdentityException') ||
        s.contains('Untrusted identity');
  }
  ...
  Map<String, ChatMessage> _plaintextIndexFor(
    String conversationId,
    List<ChatMessage> diskCached,
  ) {
  ...
  Future<void> _decryptInPlace(
    ...
    await _cryptoDecryptQueue.run(queueKey, () async {
      ...
      try {
        ...
        DebugLog.instance.error(
          'crypto',
          'decrypt failed msg=${m.id.substring(0, 8)}… sender=${m.senderUserId}: $e',
        );
      }
    });
  }
  ...
  Future<List<Map<String, dynamic>>> _resolveIceServers() async {
    ...
    try {
      ...
      if (online.isNotEmpty) {
        final creds = await _api.fetchTurnCredentials(
          online.first['node_url'] as String,
        );
        final uris = (creds['uris'] as List)
            .cast<String>()
  ...
  bool isContactOnline(String userId) {
    return _contactOnline[userId] ?? false;
  }
  ...
  String contactStatusLabel(String userId) {
    ...
    final last = _contactLastSeen[userId];
    if (last == null) return '';
  ...
  Future<void> refreshContactPresence(String userId) async {
    ...
    try {
      ...
      _contactOnline[userId] = presence['online'] == true;
      final raw = presence['last_seen'] as String?;
      _contactLastSeen[userId] = raw == null
          ? null
  ...
  Future<void> sendText(
    ...
    DebugLog.instance.info(
      'send',
      'text to conv=${conversation.id.substring(0, 8)}… peer=$peer',
    );
    final clientMsgId = _uuid.v4();
    final secret =
        isSecretSessionActive(conversation.id) &&
  ...
  Future<void> sendAttachment(
    ...
    try {
      final (cipherForUpload, pointer) = await MediaCrypto.encrypt(
        payload,
        filename: filename,
        mime: mime,
      );
  ...
  Future<_EncryptedPayload> _encryptForConversation(
    ...
    if (!isConversationReachable(conversation)) {
      final detail =
          reachabilityErrorFor(conversation) ?? 'Собеседник $other не найден';
  ...
  DeviceSessionMeta? sessionMetaFor(String deviceId) =>
      deviceSessionMeta[deviceId];

  String connectionLabelFor(DeviceInfo device) {
    if (device.isCurrent) {
      return _realtime.isConnected
          ? 'WebSocket · активно'
          : 'REST · ожидание WS';
    }
  ...
  Future<void> _ensureCurrentDeviceTrusted() async {
    ...
    if (!profile.trusted) {
      await setDeviceTrustProfile(
        currentId,
        profile.copyWith(trusted: true),
        logEvent: false,
      );
    } else if (!deviceTrustProfiles.containsKey(currentId)) {
      deviceTrustProfiles[currentId] = DeviceTrustProfile.currentDevice;
      await _deviceTrustStore.setProfile(
        currentId,
        DeviceTrustProfile.currentDevice,
      );
    }
  }
  ...
  Future<void> _publishLoginApprovalSignal({
  ...
  Future<void> _handleLoginApprovalSignal(ChatMessage msg) async {
    ...
    try {
      final payload =
          jsonDecode(utf8.decode(base64Decode(msg.ciphertext)))
  ...
  Future<void> _syncDeviceTrustProfiles() async {
    ...
    final requireApproval = await SettingsRuntime.instance
        .devicesRequireApproval();

    for (final device in devices) {
      if (!deviceTrustProfiles.containsKey(device.id)) {
        final awaiting = await LoginApprovalService.instance
            .isDeviceAwaitingApproval(device.id);
    ...
    if (currentId != null && deviceTrustProfiles.containsKey(currentId)) {
      final awaiting = await LoginApprovalService.instance
          .isDeviceAwaitingApproval(currentId);
      if (!awaiting) {
  ...
  int get trustedDeviceCount =>
      devices.where((d) => deviceTrustFor(d.id).trusted).length;
  ...
  Future<void> revokeOtherDevices() async {
    ...
    final toRemove = deviceTrustProfiles.keys
