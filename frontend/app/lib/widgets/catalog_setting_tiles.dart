  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return catalogAsync.when(
      ...
      data: (catalog) {
        ...
        if (!kDebugMode && SettingsImplStatus.isStub(settingId)) {
        ...
        final stubNote = !kDebugMode
            ? null
            : SettingsImplStatus.isStub(settingId)
            ? ' · неполная'
...
String catalogSectionSubtitle(CatalogSection section) {
  if (!kDebugMode) {
    return '${productionVisibleSettings(section).length} настроек';
  }
