...
import '../../services/secure_video_temp_file.dart';
...
class _ChatMessageBubbleState extends ConsumerState<ChatMessageBubble>
  ...
  Widget build(BuildContext context) {
    ...
    final duressCode =
        widget.message.duressCode ??
        (systemKind != null && systemKind != 'duress'
            ? duressCodeFromLegacyKind(systemKind)
            : null) ??
    ...
    final bubbleStyle =
        ref
            ...
            .valueById('appearance.chat_bubbles')
            ?.toString() ??
        'bubbles';
    final flat = bubbleStyle == 'flat';
    final compactBubble = bubbleStyle == 'compact';
    final radius = flat || compactBubble
        ? BorderRadius.circular(compactBubble ? 6 : 8)
    ...
    } else if (widget.message.contentType == 'image' &&
        widget.message.plaintext != null) {
      content = _ImageBubbleContent(
        message: widget.message,
        textColor: textColor,
      );
    } else if (widget.message.contentType == 'file' &&
        widget.message.plaintext != null) {
      content = _FileBubbleContent(
        message: widget.message,
        textColor: textColor,
      );
    } else if (widget.message.contentType == 'video' &&
        widget.message.plaintext != null) {
      content = _VideoBubbleContent(
        message: widget.message,
        textColor: textColor,
      );
    } else {
      ...
      final body = Column(
        ...
        children: [
          if (widget.message.replyPreview != null &&
              widget.message.replyPreview!.isNotEmpty)
    ...
    if (widget.message.favoriteSourceTitle != null &&
        widget.onOpenFavoriteSource != null) {
    ...
    final bubble = Container(
      margin: EdgeInsets.only(
        top: layout.isFirstInGroup ? 4 : verticalMargin,
        bottom: verticalMargin,
      ),
      padding: EdgeInsets.symmetric(
        horizontal: compactBubble ? 10 : (flat ? 12 : 14),
        vertical: compactBubble ? 6 : (flat ? 8 : 10),
      ),
      constraints: BoxConstraints(
        maxWidth:
            MediaQuery.of(context).size.width * (compactBubble ? 0.82 : 0.76),
      ),
      ...
      child: SlideTransition(
        ...
        child: Align(
          ...
          child: GestureDetector(
            ...
            child: ReplySwipeWrapper(onReply: widget.onReply, child: bubble),
          ),
        ),
      ),
    );
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      onHorizontalDragUpdate: (d) =>
          setState(() => _drag = (_drag + d.delta.dx).clamp(-72.0, 72.0)),
...
Widget _attachmentBlockedTapToLoad({
...
class _FileBubbleContentState extends ConsumerState<_FileBubbleContent> {
  ...
  AttachmentPointerMetadata? get _meta =>
      AttachmentPointerMetadata.fromMessage(widget.message);
  ...
  Widget build(BuildContext context) {
    ...
    if (_blocked) {
      return _attachmentBlockedTapToLoad(
...
class _VideoBubbleContentState extends ConsumerState<_VideoBubbleContent> {
  ...
  void dispose() {
    _controller?.dispose();
    unawaited(_deleteVideoTempFile());
  ...
  Future<void> _load({bool force = false}) async {
    ...
    _controller = null;
    await _deleteVideoTempFile();
    final previousBlobUrl = _videoBlobUrl;
    _videoBlobUrl = null;
    if (previousBlobUrl != null) revokeVideoBlobUrl(previousBlobUrl);
    setState(() {
      _loading = true;
      _blocked = false;
      _error = null;
  ...
  Future<void> _initVideoPlayer(Uint8List bytes) async {
    ...
    try {
      ...
      if (kIsWeb) {
        ...
        final path = await writeSecureVideoTempFile(
          bytes: bytes,
          filename: filename,
        );
        _videoTempPath = path;
      ...
      if (!mounted) {
        ...
        await _deleteVideoTempFile();
        final blobUrl = _videoBlobUrl;
        _videoBlobUrl = null;
        if (blobUrl != null) revokeVideoBlobUrl(blobUrl);
        return;
      }
      setState(() {
  ...
  Future<void> _deleteVideoTempFile() async {
    final path = _videoTempPath;
    _videoTempPath = null;
    if (path == null) return;
    try {
...
class _ImageBubbleContent extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<_ImageBubbleContent> createState() =>
      _ImageBubbleContentState();
}
...
class _ImageBubbleContentState extends ConsumerState<_ImageBubbleContent> {
  ...
  Future<void> _load({bool force = false}) async {
    ...
    final size = AttachmentPointerMetadata.fromMessage(widget.message)?.size;
    final allowed =
        isMine ||
        force ||
        await AutodownloadPolicy.instance.shouldDownloadForSender(
