...
void main() {
  test(
    'offer/answer/ICE/end round-trip through the 1:1 session and decode correctly',
    () async {
      ...
      final decodedOffer = await bob.decode(
        senderUserId: 'alice',
        contentType: 'call_offer',
      ...
      final answerCiphertext = await bob.encodeAnswer(
        peerUserId: 'alice',
        callId: callId,
        sdp: 'v=0 answer-sdp',
      );
      final decodedAnswer = await alice.decode(
        senderUserId: 'bob',
        contentType: 'call_answer',
      ...
      final iceCiphertext = await alice.encodeIceCandidate(
        peerUserId: 'bob',
        callId: callId,
        candidate: candidate,
      );
      final decodedIce = await bob.decode(
        senderUserId: 'alice',
        contentType: 'call_ice_candidate',
        ciphertext: iceCiphertext,
      );
      expect(decodedIce.type, CallSignalType.iceCandidate);
      expect(decodedIce.candidate, candidate);

      final endCiphertext = await bob.encodeEnd(
        peerUserId: 'alice',
        callId: callId,
      );
      final decodedEnd = await alice.decode(
        senderUserId: 'bob',
        contentType: 'call_end',
  ...
  test('reject/cancel/busy carry only a call_id, no extra payload', () async {
    ...
    final rejectCiphertext = await bob.encodeReject(
      peerUserId: 'alice',
      callId: callId,
    );
    expect(
      (await alice.decode(
        senderUserId: 'bob',
        contentType: 'call_reject',
        ciphertext: rejectCiphertext,
      )).type,
      CallSignalType.reject,
    );

    final cancelCiphertext = await alice.encodeCancel(
      peerUserId: 'bob',
      callId: callId,
    );
    expect(
      (await bob.decode(
        senderUserId: 'alice',
        contentType: 'call_cancel',
        ciphertext: cancelCiphertext,
      )).type,
      CallSignalType.cancel,
    );

    final busyCiphertext = await bob.encodeBusy(
      peerUserId: 'alice',
      callId: callId,
    );
    expect(
      (await alice.decode(
        senderUserId: 'bob',
        contentType: 'call_busy',
        ciphertext: busyCiphertext,
      )).type,
      CallSignalType.busy,
    );
  });
  ...
  test(
    'isCallSignal distinguishes signaling content_types from regular chat content',
    () {
      expect(CallSignalingService.isCallSignal('call_offer'), isTrue);
      ...
      expect(
        CallSignalingService.isCallSignal('sender_key_distribution'),
        isFalse,
      );
    },
  );
  ...
  test(
    'decode throws for a non-call content_type instead of silently misinterpreting it',
    () async {
      final aliceCrypto = CryptoService.ephemeral();
      final bobCrypto = CryptoService.ephemeral();
      final bob = CallSignalingService(bobCrypto);
      ...
      final bobBundle = await bobCrypto.generatePublishableBundle();
      await aliceCrypto.establishSessionFromBundle('bob', bobBundle);
      final textCiphertext = await aliceCrypto.encrypt(
        'bob',
