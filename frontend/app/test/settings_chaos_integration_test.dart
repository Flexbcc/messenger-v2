...
const _discoveryUrl = String.fromEnvironment(
  'DISCOVERY_NODE_URL',
  // Docker Desktop publishes this test service on IPv4 loopback. Using
  // localhost/::1 makes the result depend on the host resolver order.
...
const _homeAdvertisedUrl = String.fromEnvironment(
...
Future<String> _resolveHomeNodeId() async {
  if (_homeNodeId case final value?) return value;
  final response = await http.get(Uri.parse('$_homeInternalUrl/health'));
  if (response.statusCode != 200) {
    throw StateError('home health failed: ${response.statusCode}');
  }
  final payload = jsonDecode(response.body) as Map<String, dynamic>;
  final value = payload['node_id']?.toString() ?? '';
  if (value.isEmpty) throw StateError('home health returned no node_id');
  return _homeNodeId = value;
}
...
Future<Map<String, dynamic>?> _discoveryUserById(String userId) async {
  final resp = await http.get(
    Uri.parse('$_discoveryUrl/registry/users/$userId'),
  );
...
Future<String> _upsertDiscoveryUser(
  ...
  final homeNodeId = await _resolveHomeNodeId();
  final body = {
    'user_id': user.userId,
    // Discovery checks ownership against the Home advertisement. The address
    // used by this test process may differ only by loopback spelling.
    'home_node_url': _homeAdvertisedUrl,
  ...
  final resp = await http.post(
    ...
    headers: {
      ...
      'X-Federation-Node-Id': homeNodeId,
    },
    body: jsonEncode(body),
  );
  if (resp.statusCode < 200 || resp.statusCode >= 300) {
    throw StateError(
      'discovery upsert failed: ${resp.statusCode} ${resp.body}',
    );
  }
  return login;
}
...
Future<int> _rawDiscoverySearch(String login) async {
  final resp = await http.get(
    Uri.parse(
      '$_discoveryUrl/registry/users/search?login=${Uri.encodeQueryComponent(login)}',
    ),
  );
...
Future<Map<String, dynamic>?> _fetchStorageProfile(String userId) async {
  final mediaAccessSecret = Platform.environment['MEDIA_ACCESS_SECRET'] ?? '';
  if (mediaAccessSecret.isEmpty) {
    return const {
      'unavailable':
          'MEDIA_ACCESS_SECRET not provided; protected Media→Home check skipped',
    };
  }
  final uri = Uri.parse(
    '$_homeInternalUrl/internal/users/$userId/storage-profile',
  );
  final resp = await http.get(
    uri,
    headers: {'X-Media-Access-Secret': mediaAccessSecret},
  );
  if (resp.statusCode != 200) return null;
  return jsonDecode(resp.body) as Map<String, dynamic>;
}
...
Future<Map<String, dynamic>> _directConv(_ChaosUser a, _ChaosUser b) async {
  return a.api.createConversation(
    type: 'direct',
    participantUserIds: [b.userId],
  );
}
...
Future<String> _receiveDecrypt(
  _ChaosUser recipient,
  _ChaosUser sender,
  String convId,
) async {
  ...
  final envelope = history.first;
  final plain = await recipient.crypto.decrypt(
    sender.userId,
...
Future<void> _applyProfileSettings(
  _ChaosUser user,
...
void _printReport() {
  ...
  const wScenario = 22;
  print(
    '${'ACTOR'.padRight(wActor)} ${'SETTING'.padRight(wSetting)} ${'SCENARIO'.padRight(wScenario)} STATUS',
  );
  print('-' * 72);
  for (final c in _report) {
    ...
    print(
      '${c.actor.padRight(wActor)} ${c.settingId.padRight(wSetting)} ${c.scenario.padRight(wScenario)} $status',
    );
...
void main() {
  ...
  test(
    'three users rotate messages and apply settings — chaos audit',
    () async {
      ...
      // ── Register triangle ───────────────────────────────────────────────────
      final alice = await _registerUser(
        name: 'Alice',
        phoneNonce: phoneBase,
        login: loginAlice,
        authByte: 11,
      );
      final bob = await _registerUser(
        name: 'Bob',
        phoneNonce: phoneBase + 1,
        login: loginBob,
        authByte: 22,
      );
      final carol = await _registerUser(
        name: 'Carol',
        phoneNonce: phoneBase + 2,
        login: loginCarol,
        authByte: 33,
      );
      ...
      for (final pair in [(alice, bob), (bob, carol), (carol, alice)]) {
      ...
      if (aliceMe['display_name'] == 'Alice Chaos $runId' &&
          aliceMe['bio'] == 'bio-alice-$runId') {
        _recordOk(
          'Alice',
          'profile.display_name',
          'GET /users/me reflects profile',
        );
      } else {
        _recordFail(
          'Alice',
      ...
      if (gotBucket == 'chaos-bucket') {
        _recordOk(
          'Bob',
          'profile-settings',
          'PUT/GET roundtrip storage.s3_bucket',
        );
      } else {
        _recordFail(
          'Bob',
      ...
      if (profile?['backend'] == 's3' &&
          (profile?['s3'] as Map?)?['bucket'] == 'chaos-bucket') {
        _recordOk(
          'Bob',
          'storage.media_location',
          'internal storage-profile → S3',
        );
      } else if (bobStorage?['unavailable'] != null) {
        _recordWarn(
          'Bob',
          'storage.media_location',
          'protected Media→Home profile check',
          bobStorage!['unavailable'].toString(),
        );
      } else {
        _recordFail(
          'Bob',
      ...
      if (!await bobRuntime.readReceiptsEnabled()) {
        _recordOk(
          ...
          'SettingsRuntime.readReceiptsEnabled=false',
        );
      } else {
      ...
      if (await bobRuntime.isBlocked(alice.userId)) {
        _recordOk(
          ...
          'SettingsRuntime.isBlocked(alice)',
        );
      } else {
        _recordFail(
          'Bob',
      ...
      if (await bobRuntime.sendKey() == 'ctrl_enter') {
        ...
        _recordWarn(
          ...
          'messages.send_key',
          'runtime',
          'UI-only until chat uses runtime',
        );
      }
      ...
      if (!await carolRuntime.incomingMessagesAllowed(
        alice.userId,
        isContact: false,
      )) {
        _recordOk(
          ...
          'SettingsRuntime blocks strangers',
        );
      } else {
        _recordFail(
          'Carol',
      ...
      if (await carolRuntime.nodeAllowRelays() == false) {
        _recordOk(
          'Carol',
          'node.allow_relays',
          'SettingsRuntime.nodeAllowRelays',
        );
      } else {
        _recordWarn(
          'Carol',
          'node.allow_relays',
          'runtime',
          'node routing not exercised in test',
        );
      }
      ...
      if (aliceDiscBefore != null && aliceDiscBefore['login'] != null) {
        _recordOk(
          'Alice',
          'discovery.publish',
          'home→discovery user record present',
        );
      } else {
        _recordWarn(
          'Alice',
          'discovery.publish',
          'home→discovery',
          'not visible before manual sync',
        );
      }
      ...
      if (searchAlice == 200) {
        ...
        _recordFail(
          ...
          'privacy.username_search',
          'discovery search',
          'HTTP $searchAlice login=$loginAlice',
        );
      }
      ...
      if (searchAlice == 200 && searchCarol == 403) {
        _recordOk(
          'all',
          'privacy.username_search',
          'search policy isolated per user',
        );
      } else {
        _recordFail(
          'all',
          'privacy.username_search',
          'isolation',
          'alice=$searchAlice carol=$searchCarol',
        );
      }

      _printReport();

      final failures = _report
          .where((c) => c.status == CheckStatus.fail)
          .length;
      expect(
        failures,
        0,
        reason: 'chaos audit has $_report failures — see report above',
      );
    },
