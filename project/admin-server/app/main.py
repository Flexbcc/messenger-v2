...
from app.secrets import (
    ...
    merge_node_secrets,
    merge_storage_secrets,
    read_full_config_for_api,
)
...
from app.registry_monitor import router as registry_monitor_router
from app import operator_transport
from app import operator_ssh
from pydantic import BaseModel

ADMIN_PANEL_SECRET = os.environ.get("ADMIN_PANEL_SECRET", "")
_console_path_value = os.environ.get("ADMIN_CONSOLE_PATH", "").strip().strip("/")
ADMIN_CONSOLE_PATH = f"/{_console_path_value}" if _console_path_value else ""
...
def _media_admin_headers() -> dict[str, str]:
    secret = os.environ.get("MEDIA_ADMIN_SECRET", "").strip()
    if not secret:
        raise HTTPException(
            status_code=503,
            detail="MEDIA_ADMIN_SECRET is required for Media Node administration",
        )
    return {"X-Media-Admin-Secret": secret}
...
app.include_router(registry_monitor_router)


@app.middleware("http")
async def admin_panel_guard(request: Request, call_next):
    # Optional unguessable mount path. It is defence in depth only: the owner
    # key below remains mandatory even when somebody learns this URL.
    incoming_path = request.scope.get("path", request.url.path)
    if ADMIN_CONSOLE_PATH and incoming_path != "/health":
        if incoming_path != ADMIN_CONSOLE_PATH and not incoming_path.startswith(f"{ADMIN_CONSOLE_PATH}/"):
            return Response(status_code=404)
        stripped = incoming_path[len(ADMIN_CONSOLE_PATH):] or "/"
        request.scope["path"] = stripped
        request.scope["raw_path"] = stripped.encode("utf-8")
    path = request.scope.get("path", request.url.path)

    # HTML/CSS/JS панели меняются часто во время эксплуатации. Не позволяем
    # браузеру держать старый HTML, который ссылается на уже устаревший CSS.
    ...
    # Require the secret on every API request, including reads. Network
    # binding is useful defence in depth but is not authentication: a reverse
    # proxy or an accidental port publication must not expose cluster control.
    if is_insecure_service_secret(ADMIN_PANEL_SECRET):
        return Response(
            content='{"detail":"Admin panel is not securely configured"}',
            status_code=503,
...
async def _security_banner() -> None:
    """Refuse to expose cluster administration without real authentication."""
    if is_insecure_service_secret(ADMIN_PANEL_SECRET):
        raise RuntimeError(
            "ADMIN_PANEL_SECRET must contain at least 32 bytes and must not use a development default"
        )
    for name in ("MEDIA_ADMIN_SECRET", "OWNER_PANEL_SECRET"):
        service_secret = os.environ.get(name, "").strip()
        if service_secret and secrets_mod.compare_digest(
            ADMIN_PANEL_SECRET, service_secret
        ):
            raise RuntimeError(f"ADMIN_PANEL_SECRET and {name} must be different")
    print("[admin] ✓ ADMIN_PANEL_SECRET задан — /api/ защищён")
    if ADMIN_CONSOLE_PATH:
        print("[admin] ✓ панель опубликована на отдельном скрытом пути", flush=True)
...
def get_config():
    ...
    payload["meta"] = {
        ...
        "title": "Bootstrap-координатор" if variant == "main" else "Project-стек",
    }
    return payload
...
async def monitor_local_snapshot():
    ...
    try:
        home_url = validated_service_origin(
            os.environ.get("HOME_NODE_URL", "http://home-node:8001"),
            "HOME_NODE_URL",
        )
    except ValueError as exc:
        raise HTTPException(status_code=503, detail=str(exc)) from exc
    owner_panel_secret = os.environ.get("OWNER_PANEL_SECRET", "").strip()
    if not owner_panel_secret:
        raise HTTPException(
            status_code=503,
            detail="OWNER_PANEL_SECRET is required to read Home Node monitor data",
        )
    try:
        async with httpx.AsyncClient(
            ...
            resp = await client.get(
                f"{home_url}/monitor/snapshot",
                headers={"X-Owner-Panel-Secret": owner_panel_secret},
            )
            resp.raise_for_status()
            return resp.json()
    except httpx.HTTPStatusError as e:
...
async def reload_media_config():
    ...
    try:
        media_url = validated_service_origin(
            os.environ.get("MEDIA_NODE_URL", "http://media-node:8004"),
            "MEDIA_NODE_URL",
        )
    except ValueError as exc:
        raise HTTPException(status_code=503, detail=str(exc)) from exc
    headers = _media_admin_headers()
    try:
        async with httpx.AsyncClient(
            timeout=5.0, follow_redirects=False, trust_env=False
        ) as client:
            resp = await client.post(
                f"{media_url.rstrip('/')}/admin/reload-config",
                headers=headers,
            )
            resp.raise_for_status()
            return resp.json()
...
async def trigger_backup():
    ...
    try:
        async with httpx.AsyncClient(
            ...
            resp = await client.post(
                f"{media_url.rstrip('/')}/admin/backup",
                headers=headers,
            )
            resp.raise_for_status()
            return resp.json()
...
@app.get("/operator")
def operator_page():
    """Operator Console exists only for the dedicated operator deployment."""
    if ADMIN_VARIANT != "operator":
        return RedirectResponse(url="/nodes", status_code=307)
    return FileResponse(ADMIN_STATIC / "operator.html")


@app.get("/health-dashboard")
def health_dashboard_page():
    """The live registry supersedes the old browser-local health page."""
    return RedirectResponse(url="/nodes", status_code=307)
...
async def enrollment_proxy(
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail="invalid discovery URL port") from exc
    if (
        parsed_discovery.scheme not in {"http", "https"}
        or not parsed_discovery.hostname
        or parsed_discovery.username is not None
        or parsed_discovery.password is not None
        or parsed_discovery.path not in {"", "/"}
        or parsed_discovery.query
        or parsed_discovery.fragment
    ):
        raise HTTPException(
            status_code=400,
            detail="discovery_url must be an HTTP(S) origin without credentials or a path",
        )
    ...
    discovery_host = parsed_discovery.hostname or ""
    if ":" in discovery_host:
        discovery_host = f"[{discovery_host}]"
    discovery_origin = f"{parsed_discovery.scheme}://{discovery_host}"
    if discovery_port:
        discovery_origin = f"{discovery_origin}:{discovery_port}"
    url = f"{discovery_origin}{path}"
    headers = {
