...
def _operator_gateway(name: str) -> str:
    raw = os.environ.get(name, "").strip()
    if not raw:
        return ""
    try:
        origin = validated_service_origin(raw, name)
    except ValueError as exc:
        raise RuntimeError(str(exc)) from exc
    if not origin.startswith("https://"):
        raise RuntimeError(f"{name} must use https for operator mTLS")
    return origin


DISCOVERY_GATEWAY = _operator_gateway("OPERATOR_DISCOVERY_URL")
HOME_GATEWAY = _operator_gateway("OPERATOR_HOME_URL")
...
def _build_ssl_context() -> ssl.SSLContext:
    ...
    # A CA-valid certificate for another node must not authenticate this host.
    # Certificates therefore need a SAN matching the configured gateway name.
