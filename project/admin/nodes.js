(function () {
  ...
  async function runTrustAction(action, nodeId) {
    ...
    if (action === "suspend" || action === "compromise") {
      const decision = await AdminDialog.confirm({
        title: "Действие с доверием",
        message: action === "suspend"
          ? `Приостановить ${nodeId}?`
          : `Отозвать доступ ${nodeId}?`,
      });
  ...
  async function init() {
    try {
      ...
      if (cfg.meta?.admin_variant === "main") {
        ...
        if (sub) sub.textContent = `Bootstrap-координатор · ${cfg.node?.cluster_id || "operator-main"}`;
      }
    } catch (_) { /* ignore */ }
