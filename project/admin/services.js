    ...
    container.innerHTML = items.map((svc) => {
      ...
      const actions = compact ? "" : (svc.running
        ? `<div class="service-actions">
            <button type="button" class="btn-secondary btn-xs svc-btn" data-svc="${escapeHtml(svc.name)}" data-act="restart">Перезапустить</button>
            <button type="button" class="btn-ghost btn-xs svc-btn" data-svc="${escapeHtml(svc.name)}" data-act="stop">Остановить</button>
          </div>`
        : `<div class="service-actions">
            <button type="button" class="btn-primary btn-xs svc-btn" data-svc="${escapeHtml(svc.name)}" data-act="start">Запуск</button>
          </div>`);
      return `
        <article class="service-card service-${escapeHtml(svc.role)} ${stateCls}${compact ? " service-card-compact" : ""}">
    ...
    container.querySelectorAll(".svc-btn").forEach((btn) => {
      btn.addEventListener("click", async () => {
        ...
        try {
          ...
          await AdminDialog.info("Не удалось выполнить действие", e.message);
        } finally {
          btn.disabled = false;
        }
      });
