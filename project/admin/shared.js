...
/** Встроенные диалоги вместо browser confirm/prompt/alert. */
const AdminDialog = {
  confirm({ title = "Подтвердите действие", message = "", reasonLabel = "" } = {}) {
    return new Promise((resolve) => {
      const overlay = document.createElement("div");
      overlay.className = "admin-auth-overlay";
      overlay.innerHTML = `
        <form class="admin-auth-dialog admin-confirm-dialog">
          <div><h2>${AdminUi.escapeHtml(title)}</h2><p>${AdminUi.escapeHtml(message)}</p></div>
          ${reasonLabel ? `<label class="field-label" for="adminDialogReason">${AdminUi.escapeHtml(reasonLabel)}</label><input id="adminDialogReason" class="admin-auth-input" type="text" required>` : ""}
          <div class="admin-auth-actions">
            <button type="button" class="btn-secondary" data-dialog-cancel>Отмена</button>
            <button type="submit" class="btn-primary">Подтвердить</button>
          </div>
        </form>`;
      document.body.appendChild(overlay);
      const finish = (confirmed) => {
        const reason = overlay.querySelector("#adminDialogReason")?.value.trim() || "";
        overlay.remove();
  ...
  info(title, text) {
    return new Promise((resolve) => {
      ...
      overlay.className = "admin-auth-overlay";
      overlay.innerHTML = `
        <div class="admin-auth-dialog admin-info-dialog">
          <div><h2>${AdminUi.escapeHtml(title)}</h2></div>
          <pre class="admin-dialog-content">${AdminUi.escapeHtml(text)}</pre>
          <div class="admin-auth-actions"><button type="button" class="btn-primary" data-dialog-close>Закрыть</button></div>
        </div>`;
      document.body.appendChild(overlay);
      overlay.querySelector("[data-dialog-close]").addEventListener("click", () => {
        overlay.remove();
        resolve();
      });
    });
...
const AdminAuth = {
  ...
  /** Показать встроенный защищённый вход. Возвращает Promise<boolean>. */
  prompt(message) {
    if (this._promptPromise) return this._promptPromise;

    this._promptPromise = new Promise((resolve) => {
      const overlay = document.createElement("div");
      overlay.className = "admin-auth-overlay";
      overlay.innerHTML = `
        <form class="admin-auth-dialog" id="adminAuthForm">
          <div class="admin-auth-icon" aria-hidden="true">⌁</div>
          <div>
            <h2>Вход в панель управления</h2>
            <p>${AdminUi.escapeHtml(message || "Введите пароль администратора для доступа к данным нод.")}</p>
          </div>
          <label class="field-label" for="adminAuthSecret">Пароль администратора</label>
          <input id="adminAuthSecret" class="admin-auth-input" type="password"
                 autocomplete="current-password" required autofocus>
          <div class="admin-auth-actions">
            <button type="button" class="btn-secondary" id="adminAuthCancel">Отмена</button>
      ...
      overlay.querySelector("form").addEventListener("submit", (event) => {
        event.preventDefault();
        const entered = overlay.querySelector("input").value.trim();
        if (!entered) return;
        this.set(entered);
        finish(true);
      });
      overlay.querySelector("#adminAuthCancel").addEventListener("click", () => finish(false));
      requestAnimationFrame(() => overlay.querySelector("input").focus());
    });
  ...
  renderLockedBanner() {
    ...
    banner.className = "admin-auth-banner";
    banner.innerHTML =
      "<span>Панель заблокирована — нужен ADMIN_PANEL_SECRET</span>" +
      '<button id="adminAuthRetry" type="button">Ввести</button>';
    document.body.appendChild(banner);
    document.body.classList.add("admin-auth-locked");
    document.getElementById("adminAuthRetry").addEventListener("click", async () => {
      if (await AdminAuth.prompt()) location.reload();
    });
  },
};
...
(function installAuthInterceptor() {
  ...
  window.fetch = async function (input, init = {}) {
    ...
    if (isAdminApi && res.status === 401 && !init.__authRetried) {
      ...
      AdminAuth.clear();
      const ok = await AdminAuth.prompt(
        hadSecret
          ? "Пароль неверный. Введите пароль администратора ещё раз."
          : "Панель защищена. Введите пароль администратора."
      );
      if (ok) {
        const headers = new Headers(init.headers || {});
      ...
      // Every Admin API endpoint is JSON. Returning plain text here made
      // pages that correctly call response.json() fail with an irrelevant
      // "Unexpected token" parser error after the operator cancelled login.
      return new Response(JSON.stringify({
        detail: "Панель заблокирована — войдите для просмотра",
