...
class _DevicesScreenState extends ConsumerState<DevicesScreen> {
  ...
  Future<void> _confirmEndOtherSessions() async {
    ...
    if (others == 0) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Других активных сеансов нет')));
      return;
    }
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text('Будет удалено устройств: $others. Текущее устройство останется в системе.'),
        actions: [
    ...
    try {
      ...
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Другие сеансы завершены')));
    } catch (e) {
                  ...
                  child: ListView(
                    ...
                    children: [
                      ...
                      if (devices.isEmpty)
                        Padding(
                          padding: const EdgeInsets.all(AppSpacing.screenPadding),
                          child: Text('Нет зарегистрированных устройств', style: text.caption),
                        )
                      else
                        AppSettingsGroup(
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return AppTile(
      leading: Container(
        ...
        child: Icon(deviceTypeIcon(device.deviceType), color: colors.textPrimary, size: 20),
      ),
