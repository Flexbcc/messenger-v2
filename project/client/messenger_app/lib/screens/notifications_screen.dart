...
class NotificationsScreen extends ConsumerWidget {
  ...
  static const _channelOptions = ['Все сообщения', 'Только упоминания', 'Выключено'];
        ...
        children: [
            ...
            child: AppCard(
              child: Text(
                PlatformCapabilities.isWeb
                    ? 'Push в фоне на web — через Notification API браузера (кнопка ниже). '
                        'Remote push с сервера пока не подключён.'
          ...
          if (PlatformCapabilities.browserNotificationsAvailable) ...[
            ...
            AppSettingsGroup(
              ...
              children: [
                AppTile(
                  leading: Icon(Icons.notifications_active_outlined, color: colors.textSecondary),
                  title: 'Разрешить уведомления',
                  subtitle: OsNotificationService.instance.permission == 'granted'
                      ? 'Разрешено'
                      : 'Нужно для оповещений, когда вкладка в фоне',
                  trailing: AppTile.chevron(context),
                  onTap: () async {
                    final ok = await OsNotificationService.instance.requestPermission();
                    if (context.mounted) {
                      ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(
                          content: Text(ok ? 'Уведомления включены' : 'Разрешение не получено'),
                        ),
                      );
                    }
                  },
          ...
          AppSettingsGroup(
            ...
            children: [
              AppSwitchTile(
                leading: Icon(Icons.volume_up_outlined, color: colors.textSecondary),
                title: 'Звуки',
                value: settings.sounds,
                onChanged: settings.setSounds,
              ),
              AppSwitchTile(
                leading: Icon(Icons.vibration_outlined, color: colors.textSecondary),
                title: 'Вибрация',
          ...
          AppSettingsGroup(
            ...
            children: [
              ...
              AppTile(
                leading: Icon(Icons.groups_outlined, color: colors.textSecondary),
                title: 'Группы',
              ...
              AppTile(
                leading: Icon(Icons.visibility_off_outlined, color: colors.secondary),
                title: 'Hidden Chats',
