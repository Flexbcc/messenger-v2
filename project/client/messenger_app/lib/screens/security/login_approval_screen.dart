  ...
  Future<void> _deny(LoginApprovalRequest request) async {
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text('Устройство «${request.deviceName}» не получит доступ. Если сеанс уже создан — завершите его в списке устройств.'),
        actions: [
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Вход отклонён')));
    }
  }
              ...
              Row(
                children: [
                  Container(
                    ...
                    child: Icon(deviceTypeIcon(request.deviceType), size: 20, color: colors.textPrimary),
                  ),
                  ...
                  AppSecurityBadge(icon: Icons.schedule, label: 'Новый', color: colors.warning),
                ],
              ),
