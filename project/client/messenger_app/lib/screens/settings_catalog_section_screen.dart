...
class SettingsCatalogSectionScreen extends ConsumerWidget {
    ...
    return catalogAsync.when(
      ...
      data: (catalog) {
        ...
        if (section == null) {
          return Scaffold(
            appBar: AppBar(),
            body: Center(child: Text('Раздел не найден: $sectionId', style: text.body)),
          );
        }
        if (!values.loaded) {
        ...
        final runtime = runtimeAsync.valueOrNull ?? const CatalogRuntimeValues();
        var visible = section.settings.where(values.isVisible).toList();
        if (sectionId == 'developer' && values.valueById('developer.enabled') != true) {
          visible = visible
              .where((s) => s.id == 'developer.enabled' || s.id == 'developer.protocol_version')
              .toList();
        }
          ...
          body: ListView(
            ...
            children: [
              ...
              for (var i = 0; i < visible.length; i++) ...[
                _buildSetting(context, ref, values, runtime, actions, visible[i]),
    ...
    if (def.isSecret) {
      return AppCard(
        ...
        child: AppTile(
          title: def.title,
          titleWidget: titleW,
          subtitle: def.description.isEmpty ? 'Нажмите для настройки' : def.description,
      ...
      case 'multi_select':
        ...
        return AppCard(
          ...
          child: AppTile(
            title: def.title,
            titleWidget: titleW,
            subtitle: selected.isEmpty ? _subtitle(def, fallback: 'Не выбрано') : selected.map((e) => _labelForOption(def, e)).join(', '),
            trailing: AppTile.chevron(context),
            onTap: () async {
      ...
      case 'number':
        return AppCard(
          ...
          child: AppTile(
            ...
            onTap: () async {
              final entered = await _editText(context, def, value?.toString() ?? '', number: true);
              if (entered != null) {
      ...
      case 'text':
        return AppCard(
          ...
          child: AppTile(
            ...
            titleWidget: titleW,
            subtitle: _subtitle(def),
            trailingText: (value?.toString().isEmpty ?? true) ? 'задать' : value.toString(),
            onTap: () async {
              final entered = await _editText(context, def, value?.toString() ?? '');
      ...
      case 'list':
        return AppCard(
          ...
          child: AppTile(
            title: def.title,
            titleWidget: titleW,
            subtitle: _subtitle(def, fallback: 'Нажмите для редактирования списка'),
            trailing: AppTile.chevron(context),
  ...
  Future<String?> _pickSingle(BuildContext context, SettingDef def, String? current) {
          ...
          child: Column(
            ...
            children: [
              if (_friendlyDescription(def) != null) ...[
                Text(_friendlyDescription(def)!, style: context.textStyles.caption),
                const SizedBox(height: AppSpacing.md),
              ],
  ...
  Future<List<String>?> _pickMulti(BuildContext context, SettingDef def, List<String> current) {
  ...
  Future<String?> _editText(BuildContext context, SettingDef def, String current, {bool number = false}) {
