  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return catalogAsync.when(
      ...
      data: (catalog) {
        ...
        final stubNote = SettingsImplStatus.isStub(settingId) ? ' · только UI' : null;
        return AppCard(
          ...
          child: AppSwitchTile(
            ...
            value: value,
            onChanged: (v) => ref.read(settingsCatalogValuesProvider).setValue(def, v),
          ),
        );
      },
    );
...
String catalogSectionSubtitle(CatalogSection section) {
  ...
  final live = SettingsImplStatus.liveCount(ids);
  if (stubs == 0) return '${section.settings.length} · все активны';
  return '${section.settings.length} · $live активных, $stubs*';
}
