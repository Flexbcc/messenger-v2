...
void main() {
  test('One sender-key encryption is decryptable by every member who received the distribution', () async {
    ...
    final distributionB64 = await alice.createGroupSenderKeyDistribution(groupId, 'alice');
    await bob.processGroupSenderKeyDistribution(groupId, 'alice', distributionB64);
    await carol.processGroupSenderKeyDistribution(groupId, 'alice', distributionB64);

    const plaintext = 'hello group, one ciphertext for everyone';
    final ciphertext = await alice.encryptGroup(groupId, 'alice', utf8.encode(plaintext));

    expect(utf8.decode(await bob.decryptGroup(groupId, 'alice', ciphertext)), equals(plaintext));
    expect(utf8.decode(await carol.decryptGroup(groupId, 'alice', ciphertext)), equals(plaintext));
    ...
    expect(threw, isTrue, reason: 'Eve never received the distribution and must not be able to decrypt');
    ...
    final ciphertext2 = await alice.encryptGroup(groupId, 'alice', utf8.encode(second));
    expect(utf8.decode(await bob.decryptGroup(groupId, 'alice', ciphertext2)), equals(second));
  });
}
