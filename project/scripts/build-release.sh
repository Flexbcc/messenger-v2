...
if [[ -n "${RELEASE_SIGNING_PRIVATE_KEY:-}" ]]; then
  ...
  if [[ "${ALLOW_LEGACY_RELEASE_HMAC:-false}" != "true" ]]; then
    echo "RELEASE_SIGNING_SECRET is legacy-only; set RELEASE_SIGNING_PRIVATE_KEY" >&2
    exit 1
  fi
  SIGN_ARGS+=(--algorithm hmac --secret "$RELEASE_SIGNING_SECRET")
else
  if [[ "${ALLOW_UNSIGNED_RELEASE:-false}" != "true" ]]; then
    echo "RELEASE_SIGNING_PRIVATE_KEY is required" >&2
    exit 1
  fi
  echo "warning: explicitly creating an unsigned release" >&2
  RELEASE_SIGNATURE=""
fi
