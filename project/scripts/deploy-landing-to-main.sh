...
[[ -f "$MSG/pubspec.yaml" ]] || { echo "Canonical Flutter client not found at $MSG" >&2; exit 1; }
...
"${RSYNC[@]}" --delete \
  ...
  "$LANDING/" "$MAIN:$REMOTE_DIR/"

# Remove legacy broken PWA path if present
"${SSH[@]}" "$MAIN" "rm -rf '$REMOTE_DIR/downloads/messenger-web'"
