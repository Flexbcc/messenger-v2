...
class ClusterRun:
    def __init__(self) -> None:
        ...
        self.discovery_admin_secret = uuid.uuid4().hex + uuid.uuid4().hex
    ...
    def add_service(self, name: str, role: str, env: dict[str, str]) -> Service:
        env = {
            **env,
            # The harness verifies server TLS with its isolated CA. mTLS is a
            # separate test concern and must not leak in from the caller's .env.
            "MTLS_MODE": "off",
            # This isolated harness intentionally uses legacy auto-enrollment;
            # production remains fail-closed unless explicitly configured.
            "ALLOW_INSECURE_DISCOVERY_MODES": "true",
            "DISCOVERY_ADMIN_SECRET": self.discovery_admin_secret,
    ...
    def common_node_env(self, name: str) -> dict[str, str]:
        ...
        discovery_signing_keys = ",".join(
            httpx.get(
                f"{self.services[discovery_name].url}/discovery-pubkey",
                timeout=5,
            ).json()["public_key"]
            for discovery_name in ("discovery", "discovery-d2", "discovery-d3")
        )
        return {
            "DISCOVERY_NODE_URL": self.discovery_url,
            "ENROLLMENT_MODE": "legacy",
            "INTERNAL_SECURITY_MODE": "signed",
            "DISCOVERY_SIGNING_PUBLIC_KEYS": discovery_signing_keys,
    ...
    def identity_node_id(self, name: str) -> str:
        certificate = json.loads(
            (
                self.run_dir
                / "data"
                / name
                / "operational-certificate.json"
    ...
    def deliver_payload(self, origin: str, target_url: str, packet_id: str) -> dict:
        ...
        origin_node_id = self.identity_node_id(origin)
        target_name = next(
            (name for name, service in self.services.items() if service.url == target_url),
            None,
        )
        recipient_user_id = (
            "user-recipient"
            if target_name == "home-b"
            else f"user-{target_name}-recipient"
        )
        ...
        conversation_meta = {
            ...
            "type": "direct",
            "name": None,
            "participant_user_ids": [f"user-{origin}", recipient_user_id],
        }
    ...
    def send_direct(self, origin: str, target: Service, packet_id: str) -> httpx.Response:
        ...
        return self.signed_request(
            ...
            node_id=self.identity_node_id(origin),
    ...
    def start_cluster(self) -> None:
        ...
        for suffix in ("d2", "d3"):
            ...
            registration_fields = federation_registration_fields(
                ...
                operational_credential_chain_path=replica.env[
                    "NODE_OPERATIONAL_CREDENTIAL_CHAIN_PATH"
                ],
                transport_key_path=replica.env["NODE_TRANSPORT_KEY_PATH"],
                transport_certificate_path=replica.env[
            ...
            if response.status_code >= 400:
                raise RuntimeError(
                    "Discovery replica registration failed: "
            ...
            replica.env.update(
                {
                    ...
                    "NODE_ADVERTISEMENT_GOSSIP_INTERVAL_SECONDS": "5",
            ...
            home.env.update(
                {
                    ...
                    # ``docker compose run`` inherits the service's plural
                    # bootstrap list. Keep the harness hermetic instead of
                    # accidentally selecting the long-running Compose Storage.
                    "STORAGE_NODE_URLS": storage.url,
        ...
        # Every Home gets a real local recipient. This lets later failover and
        # acknowledgement checks target different Homes without pretending the
        # same UserID is owned by several nodes.
        for index, name in enumerate(("home-a", "home-b", "home-c"), start=1):
            recipient_id = (
                "user-recipient" if name == "home-b" else f"user-{name}-recipient"
            )
            with sqlite3.connect(
                self.run_dir / "data" / name / "home.db"
            ) as connection:
                connection.execute(
                    """INSERT OR IGNORE INTO users
                       (id, display_name, phone, password_hash, created_at)
                       VALUES (?, ?, ?, ?, ?)""",
                    (
                        recipient_id,
                        f"TLS test recipient {name}",
                        f"+1000000000{index}",
                        "not-used-by-cluster-test",
        ...
        refreshed_fields = federation_registration_fields(
            ...
            home_a_env["NODE_CAPABILITY_CERTIFICATE_PATH"],
            capability_authority_state_path=str(
                self.capability_authority_state_path
            ),
            operational_credential_chain_path=home_a_env[
                "NODE_OPERATIONAL_CREDENTIAL_CHAIN_PATH"
            ],
            transport_key_path=home_a_env["NODE_TRANSPORT_KEY_PATH"],
            transport_certificate_path=home_a_env[
                "NODE_TRANSPORT_CERTIFICATE_PATH"
            ],
            supported_transports=("https",),
        )
        ...
        relay_refresh_fields = federation_registration_fields(
            ...
            operational_credential_chain_path=relay_env[
                "NODE_OPERATIONAL_CREDENTIAL_CHAIN_PATH"
            ],
            transport_key_path=relay_env["NODE_TRANSPORT_KEY_PATH"],
            transport_certificate_path=relay_env[
                "NODE_TRANSPORT_CERTIFICATE_PATH"
            ],
            supported_transports=("https",),
        )
    ...
    def run_checks(self) -> None:
        ...
        # Federated sender ownership is resolved through Discovery; publish the
        # synthetic sender before exercising the data plane.
        sender_record = {
            "user_id": "user-home-a",
            "home_node_url": home_a.url,
            "display_name": "TLS test sender",
            "auth_public_key": public_key_b64(SigningKey.generate()),
            "cluster_id": "local-test",
        ...
        sender_record_codes = [
            self.signed_request(
                method="POST",
                url=f"{service.url}/registry/users",
                path="/registry/users",
                node_id=self.identity_node_id("home-a"),
                key_path=self.node_key_path("home-a"),
                payload=sender_record,
            ).status_code
            for service in discovery_services
        ]
        self.record(
            "three-discovery-sender-home-publish",
            sender_record_codes == [200, 200, 200],
            f"HTTP={sender_record_codes}",
        )
        additional_owners = (
            ("home-a", "user-home-a-recipient", home_a.url),
            ("home-b", "user-home-b", home_b.url),
            ("home-b", "user-recipient", home_b.url),
            ("home-c", "user-home-c-recipient", self.services["home-c"].url),
        )
        additional_owner_codes: list[int] = []
        for owner_name, user_id, owner_url in additional_owners:
            owner_record = {
                "user_id": user_id,
                "home_node_url": owner_url,
                "display_name": f"TLS identity {user_id}",
                "auth_public_key": public_key_b64(SigningKey.generate()),
                "cluster_id": "local-test",
                "username_search_enabled": False,
            }
            additional_owner_codes.extend(
                self.signed_request(
                    method="POST",
                    url=f"{service.url}/registry/users",
                    path="/registry/users",
                    node_id=self.identity_node_id(owner_name),
                    key_path=self.node_key_path(owner_name),
                    payload=owner_record,
                ).status_code
                for service in discovery_services
            )
        self.record(
            "three-discovery-test-user-ownership",
            additional_owner_codes == [200] * len(additional_owner_codes),
            f"HTTP={additional_owner_codes}",
        )
        ...
        self.record(
            ...
            f"HTTP {direct.status_code} {direct.text}",
        )
        ...
        home_a_node_id = self.identity_node_id("home-a")
        home_b_node_id = self.identity_node_id("home-b")
        relay_direct = self.deliver_payload("home-a", home_b.url, relay_packet)
        relay_payload = build_relay_forward_payload(
            signing_key=signing_key,
            origin_node_id=home_a_node_id,
        ...
        with websocket_connect(
            ...
            for sequence in (1, 2):
                ...
                websocket_results.append(
                    ...
                    and result.get("result", {}).get("status") == "home_accepted"
                )
            websocket.send(
                encode_batch(
                    sequence=2,
        ...
        self.record(
            ...
            and all(result.get("status") == "home_accepted" for result in adapter_results),
        ...
        offline_owner_record = {
            "user_id": "offline-device",
            "home_node_url": home_a.url,
            "display_name": "TLS offline recipient",
            "auth_public_key": public_key_b64(SigningKey.generate()),
            "cluster_id": "local-test",
            "username_search_enabled": False,
        }
        offline_owner_codes = [
            self.signed_request(
                ...
                node_id=home_a_node_id,
                key_path=self.node_key_path("home-a"),
                payload=offline_owner_record,
            ).status_code
            for service in discovery_services
        ]
        self.record(
            "three-discovery-offline-owner-publish",
            offline_owner_codes == [200, 200, 200],
            f"HTTP={offline_owner_codes}",
        )
        ...
        def relay_challenge(context):
            ...
            for attempt in range(3):
                ...
                if (
                    response.status_code == 200
                    and response.json().get("status") == "home_accepted"
                ):
                    return True
                if attempt < 2:
                    time.sleep(0.25)
        ...
        def storage_challenge(context):
            ...
            challenge_owner = {
                "user_id": mailbox,
                "home_node_url": home_a.url,
                "display_name": "Synthetic Storage challenge",
                "auth_public_key": public_key_b64(SigningKey.generate()),
                "cluster_id": "local-test",
                "username_search_enabled": False,
            }
            owner_codes = [
                self.signed_request(
                    ...
                    node_id=home_a_node_id,
                    key_path=self.node_key_path("home-a"),
                    payload=challenge_owner,
                ).status_code
                for service in discovery_services
            ]
            if owner_codes != [200, 200, 200]:
                return False
            challenge_envelope = {
        ...
        ack_payload = build_delivery_ack_payload(
            ...
            origin_node_id=home_b_node_id,
            packet_id=ack_source_packet,
            conversation_id=str(uuid.uuid4()),
            from_user_id="user-home-b",
            from_device_id="tls-home-b-device",
        ...
        # Home C was deliberately revoked above and must remain unusable.
        # Exercise the freeze invariant with two still-trusted Homes instead.
        data_after_freeze = self.send_direct("home-a", home_b, str(uuid.uuid4()))
        self.record(
