...
from app.challenge_observer_access import (
    require_portable_observer_allowed,
    require_registered_observer,
)
...
from app.config import RANDOMNESS_CHECKPOINT_MODE
...
def pull_assignments(
    ...
    *,
    ...
    with get_conn() as conn:
        require_registered_observer(conn, observer_node_id, authorization)
    ...
    *,
    ...
    with get_conn() as conn:
        ...
        if observer_certificate is None:
            observer = require_registered_observer(
                conn,
                observer_node_id,
                authorization,
            )
