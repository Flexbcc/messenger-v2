...
def set_setting_on(conn: sqlite3.Connection, key: str, value: str) -> None:
    conn.execute(
        """
...
def add_blocked_version(version: str, reason: str | None, blocked_at: str) -> None:
    with get_conn() as conn:
        add_blocked_version_on(conn, version, reason, blocked_at)
        conn.commit()


def remove_blocked_version_on(conn: sqlite3.Connection, version: str) -> bool:
    cursor = conn.execute(
        "DELETE FROM blocked_versions WHERE version = ?", (version,)
    )
...
def remove_blocked_version(version: str) -> bool:
    with get_conn() as conn:
        removed = remove_blocked_version_on(conn, version)
        conn.commit()
        return removed
