...
async def _last_entry_hash(db: AsyncSession, user_id: str) -> Optional[str]:
    ...
    result = await db.execute(
        ...
        .order_by(
            KeyTransparencyLog.created_at.desc(),
            KeyTransparencyLog.id.desc(),
        )
        .limit(1)
    )
...
async def get_key_log(
    ...
    *,
    ...
    if not 1 <= limit <= 201:
        raise ValueError("invalid key log limit")
    query = (
        select(KeyTransparencyLog)
        .where(KeyTransparencyLog.user_id == user_id)
        .order_by(
            KeyTransparencyLog.created_at.asc(),
            KeyTransparencyLog.id.asc(),
        )
        .limit(limit)
    )
    if since_id:
        ...
        since_result = await db.execute(
            select(KeyTransparencyLog.created_at, KeyTransparencyLog.id)
            .where(
                KeyTransparencyLog.id == since_id,
                KeyTransparencyLog.user_id == user_id,
            )
        )
        cursor = since_result.one_or_none()
        if cursor is None:
            raise ValueError("key log cursor not found")
        since_at, cursor_id = cursor
        query = query.where(
            or_(
                KeyTransparencyLog.created_at > since_at,
                and_(
                    KeyTransparencyLog.created_at == since_at,
                    KeyTransparencyLog.id > cursor_id,
                ),
            )
        )
...
def verify_log_chain(
    entries: list[KeyTransparencyLog],
    *,
    ...
    for i, entry in enumerate(entries):
        ...
        if i == 0:
            if entry.prev_entry_hash != expected_previous_hash:
                errors.append(
                    f"Entry {entry.id}: prev_entry_hash does not match page boundary"
                )
        else:
            prev = entries[i - 1]
