...
_DEVICE_STALE_DAYS = settings.device_stale_days
_DEVICE_CLEANUP_INTERVAL = settings.device_cleanup_interval_seconds
...
_REVOKED_CLEANUP_INTERVAL = settings.revoked_tokens_cleanup_interval_seconds
...
from app.routers import auth, conversations, device_links, devices, internal, media_proxy, messages, monitor, push_subscriptions, security_signals, storage, user_devices, users, ws
from app.ws import manager
from shared.security.health import security_health_snapshot
from shared.security.body_limit import (
...
from app.federation import get_federation_counters, get_transport_runtime_status
...
def health():
    ...
    from app.delivery_metrics import delivery_timing_snapshot
    resp: dict = {
        "status": "ok",
        "node_role": "home",
        "node_id": fs.node_id,
        ...
        "load": {
            ...
            "delivery_timing": delivery_timing_snapshot(),
            "transport_policy": {
                "direct_max_connections": settings.direct_max_connections,
                "direct_keepalive_connections": settings.direct_max_keepalive_connections,
                "direct_keepalive_expiry_seconds": settings.direct_keepalive_expiry_seconds,
                "relay_mode": settings.relay_transport_mode,
                "relay_max_persistent_peers": settings.relay_max_persistent_peers,
                "peer_guard_count": settings.peer_guard_count,
                "peer_rotating_count": settings.peer_rotating_count,
                "peer_reserve_count": settings.peer_reserve_count,
                "relay_link_idle_seconds": settings.relay_link_idle_seconds,
            },
            "transport_runtime": get_transport_runtime_status(),
...
if settings.owner_panel_enabled and PANEL_DIR.is_dir():
    @app.get("/panel")
    @app.get("/panel/")
    def owner_panel():
