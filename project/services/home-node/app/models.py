...
from sqlalchemy import Boolean, String, ForeignKey, DateTime, Text, JSON, Integer, UniqueConstraint
...
class Message(Base):
    ...
    # Immutable delivery target snapshot derived from device_envelopes when
    # the packet is accepted. ACK completion must not change merely because a
    # user later adds or revokes another device.
    delivery_target_device_ids: Mapped[Optional[list]] = mapped_column(
        JSON, nullable=True
    )
    # Explicit opaque media identifiers are required only for routing the
    # associated blobs across Home nodes; media content remains encrypted.
    media_ids: Mapped[Optional[list]] = mapped_column(JSON, nullable=True)
...
class FederatedMediaRef(Base):
    ...
    media_id — канонический SHA-256 зашифрованного blob; клиент передаёт его
    явно через SendMessageRequest.media_ids[] для маршрутизации.
    """
...
class MessageMediaRef(Base):
    """Authorization index connecting a blob to the conversation carrying it."""

    __tablename__ = "message_media_refs"
...
class MessageDeliveryAck(Base):
    ...
    unique(packet_id, from_user_id, from_device_id) makes re-acking the same
    packet from the same authenticated device idempotent while allowing every
    targeted device to acknowledge independently.
    """
    ...
    __table_args__ = (
        UniqueConstraint(
            ...
            "from_user_id",
            "from_device_id",
            name="uq_delivery_ack_packet_user_device",
        ),
    )
    ...
    # Empty string is the explicit compatibility value for legacy user-level
    # envelopes. New clients always ACK with their authenticated Device ID.
    from_device_id: Mapped[str] = mapped_column(String(36), default="", nullable=False)
