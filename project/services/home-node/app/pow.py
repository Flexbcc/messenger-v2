...
# A mobile/web client must be able to solve this interactively. Values above
# six hexadecimal leading zeroes turn the anti-abuse challenge into a client
# availability attack rather than a useful throttle.
_DIFFICULTY = _bounded_env_int("REGISTRATION_POW_DIFFICULTY", 4, 0, 6)
_CHALLENGE_TTL = _bounded_env_int("POW_CHALLENGE_TTL_SECONDS", 300, 30, 900)
_MAX_PENDING_CHALLENGES = _bounded_env_int(
    "POW_MAX_PENDING_CHALLENGES", 50_000, 100, 1_000_000
)
...
def issue_challenge() -> dict:
    ...
    _lazy_cleanup()
    if len(_challenges) >= _MAX_PENDING_CHALLENGES:
        raise RuntimeError("PoW challenge capacity exhausted")
