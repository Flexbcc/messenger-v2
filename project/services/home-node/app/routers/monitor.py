...
def require_owner_panel(
    x_owner_panel_secret: str = Header("", alias="X-Owner-Panel-Secret"),
) -> None:
    if not settings.owner_panel_enabled or not settings.owner_panel_secret:
        raise HTTPException(status_code=503, detail="Owner panel API is disabled")
    if not secrets.compare_digest(x_owner_panel_secret, settings.owner_panel_secret):
        raise HTTPException(status_code=401, detail="Owner panel authentication required")


router = APIRouter(
    prefix="/monitor",
    tags=["monitor"],
    dependencies=[Depends(require_owner_panel)],
)
