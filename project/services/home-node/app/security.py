...
from shared.security.jwt_auth import verify_jwt_token

_password_hasher = PasswordHasher()
_dummy_password_hash = _password_hasher.hash(
    "fixed-process-dummy-password-for-timing-equalization"
)
...
def verify_password_or_dummy(password: str, password_hash: Optional[str]) -> bool:
    """Always execute Argon2 so unknown identifiers do not have a fast path."""
    return verify_password(password, password_hash or _dummy_password_hash)


def constant_time_bytes_equal(left: bytes, right: bytes) -> bool:
...
def verify_token(token: str) -> Optional[dict]:
    return verify_jwt_token(token, settings.jwt_secret, settings.jwt_algorithm)
