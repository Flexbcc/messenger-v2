...
from shared.security.media_id import is_valid_media_id
...
def load_blob(media_id: str) -> Optional[bytes]:
    # Defence in depth for non-HTTP callers and legacy filesystem backends.
    # Rejecting non-digests here prevents a crafted object key from becoming
    # a path traversal even if a caller bypasses the router schema.
    ...
    if not row:
        return _verified_blob(media_id, _legacy_load(media_id))
    ...
    if not isinstance(object_key, str) or not is_valid_media_id(object_key):
        return None
    data = _verified_blob(media_id, backend.get(object_key))
    if data is not None:
        with get_conn() as conn:
            conn.execute(
...
def _verified_blob(media_id: str, data: Optional[bytes]) -> Optional[bytes]:
    if data is None:
        return None
    actual_id = hashlib.sha256(data).hexdigest()
    if not hmac.compare_digest(actual_id, media_id):
        return None
    return data
