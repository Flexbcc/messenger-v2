...
class Settings:
    enabled: bool = _env_bool("NODE_PARTICIPATE_RELAY", True)
    node_id: str = os.environ.get("RELAY_NODE_ID", "relay-local")
    public_url: str = validated_service_origin(
        os.environ.get("RELAY_NODE_PUBLIC_URL", "http://localhost:8005"),
        "RELAY_NODE_PUBLIC_URL",
    )
    ...
    mix_discovery_urls: tuple[str, ...] = tuple(
        dict.fromkeys(
            validated_service_origin(value, "MIX_DISCOVERY_URLS")
    ...
    capabilities: list = ["relay"] if enabled else []
    supported_transports: tuple[str, ...] = (
        ("https", "quic")
        if _env_bool("RELAY_QUIC_ENABLED", False)
        else ("https",)
    )
    ...
    relay_rate_limit: int = _bounded_int(
        "RELAY_RATE_LIMIT", 100, minimum=1, maximum=1_000_000
    )
    relay_rate_window_seconds: int = _bounded_int(
        "RELAY_RATE_WINDOW_SECONDS", 60, minimum=1, maximum=86_400
    )
    relay_rate_max_origins: int = _bounded_int(
        "RELAY_RATE_MAX_ORIGINS", 10_000, minimum=100, maximum=1_000_000
    )
    ...
    target_validation_mode: str = os.environ.get("RELAY_TARGET_VALIDATION_MODE", "enforce").lower()
    ...
    if target_validation_mode != "enforce" and not _env_bool(
        "ALLOW_INSECURE_RELAY_TARGETS", False
    ):
        raise RuntimeError(
            "Relay target validation downgrade requires "
            "ALLOW_INSECURE_RELAY_TARGETS=true"
        )
