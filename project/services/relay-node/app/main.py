...
from app.ws_connections import WebSocketConnectionRegistry
from app.traffic_accounting import RelayTrafficAccounting
from app.relay_forwarding import (
    MAX_HOPS,
    fastest_hubs,
    list_hub_urls,
    target_is_trusted_home,
...
# ---------------------------------------------------------------------------
_traffic_accounting = RelayTrafficAccounting(
    window_seconds=settings.relay_rate_window_seconds,
    request_limit=settings.relay_rate_limit,
    max_origins=settings.relay_rate_max_origins,
)


def _check_rate_limit(origin_node_id: str) -> bool:
    """Возвращает True если лимит НЕ превышен (запрос разрешён)."""
    return _traffic_accounting.allow_request(
        origin_node_id,
...
async def _check_certified_traffic_quota(origin_node_id: str, payload: dict) -> bool:
    ...
    return _traffic_accounting.allow_certified_quota(
        origin_node_id,
        payload,
        quotas,
        now=asyncio.get_running_loop().time(),
...
_forwarded_count = 0
_ws_connections = WebSocketConnectionRegistry(
    total_limit=settings.ws_max_connections,
    per_peer_limit=settings.ws_max_connections_per_peer,
)
...
async def on_startup():
    ...
    if not settings.enabled:
        logger.info("Relay participation is disabled by operator policy")
        return
...
def health():
    ...
    return {
        ...
        "load": {
            ...
            "active_transport_connections": _ws_connections.active_total,
            "active_transport_peers": _ws_connections.active_peer_count,
            "tracked_traffic_origins": _traffic_accounting.tracked_origins,
        },
        "security": security_health_snapshot(),
        "runtime": {
            "enabled_by_operator": settings.enabled,
...
async def mix_ingress(payload: dict, origin_node_id: str = FederationAuthDep):
    if not settings.enabled:
        raise HTTPException(status_code=503, detail="Relay participation is disabled")
    fs = get_federation_security()
    if not (
...
async def forward(payload: dict, _verified: str = FederationAuthDep):
    ...
    target_url, hop_count, envelope, conversation_meta, federation = validate_forward_payload(payload)
    ...
    if settings.target_validation_mode != "off":
        target_is_trusted = await target_is_trusted_home(target_url)
    ...
    async with httpx.AsyncClient(
        ...
        try:
            ...
            target_response = parse_bounded_json_response(resp, max_bytes=16 * 1024)
            if not isinstance(target_response, dict) or target_response.get("status") != "home_accepted":
                raise RuntimeError("target Home returned invalid acceptance")
            _forwarded_count += 1
            return {"status": "home_accepted", "route": "relay_direct", "hops": hop_count}
        except (httpx.HTTPError, RuntimeError, ValueError) as direct_err:
            logger.warning(
    ...
    live_hubs = await fastest_hubs(hub_urls)
    if not live_hubs:
        raise HTTPException(
            status_code=502,
    ...
    async with httpx.AsyncClient(
        ...
        for hub_url in live_hubs:
            try:
                ...
                resp.raise_for_status()
                hub_response = parse_bounded_json_response(resp, max_bytes=16 * 1024)
                if not isinstance(hub_response, dict) or hub_response.get("status") != "home_accepted":
                    raise RuntimeError("hub returned invalid Home acceptance")
                _forwarded_count += 1
                logger.info(
                    "Multi-hop: relayed %s → hub %s → target %s",
                    origin, hub_url, target_url,
                )
                return {"status": "home_accepted", "route": "relay_hub", "hub": hub_url, "hops": hop_count + 1}
            except (httpx.HTTPError, RuntimeError, ValueError) as hub_err:
                last_err = hub_err
...
async def relay_websocket(websocket: WebSocket):
    ...
    if not settings.enabled:
        await websocket.close(code=4403, reason="relay participation disabled")
        return
    if not _ws_connections.reserve():
    ...
    try:
        ...
        if not _ws_connections.bind_peer(
        ...
        _ws_connections.release(peer_node_id if peer_slot_reserved else None)
