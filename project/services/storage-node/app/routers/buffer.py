...
from shared.security.envelope_verify import (
    transport_origin_matches,
    verify_incoming_federation,
)
...
from shared.security.user_home_record import validate_user_home_record
...
async def _require_recipient_home(recipient_user_id: str, origin_node_id: str) -> None:
    if not 1 <= len(recipient_user_id) <= 256:
        raise HTTPException(status_code=422, detail="invalid recipient id")
    fs = get_federation_security()
    origin = await fs.trust_cache.get_node(origin_node_id)
    origin_url = origin.get("node_url") if origin else None
    if not isinstance(origin_url, str) or not origin_url:
        raise HTTPException(status_code=403, detail="Home endpoint is unavailable")
    try:
        async with httpx.AsyncClient(
            ...
            response = await client.get(
                f"{settings.discovery_url.rstrip('/')}/registry/users/{recipient_user_id}"
            )
    except httpx.HTTPError as exc:
        raise HTTPException(
            status_code=503,
            detail="Discovery ownership check is unavailable",
        ) from exc
    if response.status_code != 200:
        raise HTTPException(status_code=403, detail="Recipient Home is not registered")
    try:
        record = response.json()
    except ValueError as exc:
        raise HTTPException(status_code=503, detail="Discovery returned invalid data") from exc
    home_url, error = validate_user_home_record(
        record,
        expected_user_id=recipient_user_id,
        require_signature=True,
        trusted_public_keys=settings.discovery_signing_public_keys,
    )
    if home_url is None or home_url != origin_url.rstrip("/"):
        raise HTTPException(
            status_code=403,
            detail=error or "Requesting Home does not own this recipient",
        )


@router.post("/buffer", response_model=BufferedEnvelopeResponse)
async def buffer_envelope(payload: BufferRequest, verified_origin: str = FederationAuthDep):
    ...
    inner_origin = payload.federation.get("origin_node_id") if payload.federation else None
    if not transport_origin_matches(verified_origin, payload.federation):
        raise HTTPException(
            status_code=403,
            detail="transport origin does not match buffered envelope origin",
        )
...
async def ack_delivered(entry_id: str, verified_origin: str = FederationAuthDep):
    """Release a mailbox copy after the recipient device's semantic ACK."""
    with get_conn() as conn:
        row = conn.execute(
            "SELECT recipient_device_id FROM buffered_envelopes WHERE id = ?",
            (entry_id,),
        ).fetchone()
    if row is None:
        raise HTTPException(status_code=404, detail="Not found")
    await _require_recipient_home(row["recipient_device_id"], verified_origin)
    with get_conn() as conn:
