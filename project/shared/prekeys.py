...
PREKEY_CONSUMPTION_MODE = os.environ.get("PREKEY_CONSUMPTION_MODE", "strict").lower()
if PREKEY_CONSUMPTION_MODE not in {"legacy", "strict"}:
    raise RuntimeError("PREKEY_CONSUMPTION_MODE must be legacy or strict")
ALLOW_LEGACY_PREKEY_MODE = _env_bool("ALLOW_LEGACY_PREKEY_MODE", False)
if PREKEY_CONSUMPTION_MODE == "legacy" and not ALLOW_LEGACY_PREKEY_MODE:
    raise RuntimeError(
        "legacy prekey consumption requires ALLOW_LEGACY_PREKEY_MODE=true"
    )
...
def resolve_prekey_mode(api_version: Optional[int] = None) -> str:
    """Resolve response shape without letting callers downgrade server policy."""
    if PREKEY_CONSUMPTION_MODE == "strict":
        return "strict"
    if api_version == 1:
        return "strict"
    return "legacy"
