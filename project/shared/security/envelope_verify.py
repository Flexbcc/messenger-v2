...
def transport_origin_matches(
    verified_transport_origin: str,
    federation: Optional[dict],
) -> bool:
    """Bind both authentication layers when a signed envelope is present.

    The deliberately insecure local/legacy profile has no inner federation
    block; in that mode the already verified transport identity is the only
    origin.  Signed profiles must agree exactly and fail closed.
    """
    if federation is None:
        return True
    return verified_transport_origin == federation.get("origin_node_id")
