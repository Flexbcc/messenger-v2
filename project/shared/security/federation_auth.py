...
async def verify_federation_request(
    ...
    *,
    ...
    await _verify_signature_with_rotation_refresh(
        context,
        method=request.method,
        path=effective_path,
        body=body,
...
async def _verify_signature_with_rotation_refresh(
    context: Mapping[str, str],
    *,
    method: str,
    path: str,
    ...
    try:
        ...
        if exc.status_code != 401:
            raise
    refreshed = await trust_cache.refresh_after_signature_failure()
    refreshed_key = (
        await trust_cache.signing_public_key(context["node_id"])
        if refreshed
        else None
    )
