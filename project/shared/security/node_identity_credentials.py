...
def load_operational_credential_chain(path: str) -> list[dict[str, Any]]:
    """Load and fully validate a portable Root-signed credential history.

    Registration may use this bounded chain to bootstrap a fresh independent
    Discovery replica. Regular heartbeats continue to carry only the head.
    """
    if not path:
        return []
    states = _read_operational_state_chain(Path(path))
    if states is None:
        return []
    try:
        node_id = states[0]["node_id"]
    except (KeyError, TypeError, IndexError) as exc:
        raise ValueError("invalid Operational Credential chain") from exc
    if not isinstance(node_id, str) or not node_id:
        raise ValueError("invalid Operational Credential chain NodeID")
