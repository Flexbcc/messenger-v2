...
class RelayWebSocketClient:
    ...
    def __init__(
        ...
        *,
        ...
        if not 1 <= max_links <= 64:
            raise ValueError("max_links must be between 1 and 64")
        if not 10 <= idle_timeout_seconds <= 3600:
            raise ValueError("idle_timeout_seconds must be between 10 and 3600")
    ...
    def status_snapshot(self) -> dict[str, int]:
        """Return privacy-safe link counts without exposing Relay addresses."""
        now = time.monotonic()
        active = sum(
            1
            for link in self._links.values()
            if now - link.last_used < self.idle_timeout_seconds
        )
        return {
            "active_links": active,
    ...
    async def _get_link(self, relay_url: str) -> _RelayLink:
        to_close: list[_RelayLink] = []
        async with self._links_lock:
            now = time.monotonic()
            for url, candidate in list(self._links.items()):
                if now - candidate.last_used >= self.idle_timeout_seconds:
                    self._links.pop(url, None)
                    to_close.append(candidate)
            link = self._links.get(relay_url)
            if link is None:
                if len(self._links) >= self.max_links:
                    oldest_url, oldest = min(
                        self._links.items(), key=lambda item: item[1].last_used
                    )
                    self._links.pop(oldest_url, None)
