...
def test_transport_origin_binding_supports_legacy_and_fails_closed_for_signed():
    assert transport_origin_matches("home-1", None)
    assert transport_origin_matches("home-1", {"origin_node_id": "home-1"})
    assert not transport_origin_matches(
        "home-1", {"origin_node_id": "attacker-home"}
    )
    assert not transport_origin_matches("home-1", {})
