...
def trust_cache(signing_key: SigningKey):
    ...
    cache.refresh_after_signature_failure = AsyncMock(return_value=False)
    return cache
...
@pytest.mark.asyncio
async def test_signed_request_retries_once_after_operational_key_rotation(monkeypatch):
    import shared.security.federation_auth as fed_auth
    ...
    cache.is_trusted = AsyncMock(return_value=True)
    cache.has_capability = AsyncMock(return_value=True)
    cache.signing_public_key = AsyncMock(
        side_effect=[public_key_b64(old_key), public_key_b64(new_key)]
    )
    cache.refresh_after_signature_failure = AsyncMock(return_value=True)

    node_id = await verify_federation_headers(
        headers,
        method="POST",
        path="/buffer",
        body=body,
    ...
    assert node_id == "ouo-node-v1-stable"
    cache.refresh_after_signature_failure.assert_awaited_once()
