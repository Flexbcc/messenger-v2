...
def test_report_mode_signs_as_verified_identity_when_credentials_exist(tmp_path):
    security = FederationSecurity(
        discovery_url="https://discovery.example",
        node_id="legacy-alias",
        signing_key_path=str(tmp_path / "operational.key"),
    ...
    assert security.node_id == security.identity_node_id
    assert security.node_id.startswith(NODE_ID_PREFIX)
    assert security.node_id != security.node_alias
