...
class MetaDb {
  ...
  static Future<MetaDb> open(String allowedRoot) async {
    ...
    db.execute('CREATE INDEX IF NOT EXISTS idx_peers_pubkey ON peers(pubkey);');
  ...
  /// A signature is authorized only when both the claimed node id and key
  /// belong to the same active pairing record.
  bool isPairedIdentity(String userUuid, String pubkey) {
    final rs = _db.select(
      '''SELECT 1 FROM peers
         WHERE user_uuid = ? AND pubkey = ? AND revoked = 0 LIMIT 1;''',
      [userUuid, pubkey],
    );
    return rs.isNotEmpty;
  }
  ...
  void revokePeer(String userUuid) {
    _db.execute('UPDATE peers SET revoked = 1 WHERE user_uuid = ?;', [
      userUuid,
    ]);
  }
  ...
  static const _activeBlob = "refcount > 0 AND state = 'present'";
  ...
  List<({String userUuid, String hash})> collectGarbage({int ttlDays = 0}) {
    ...
    final removed = rs
        .map(
          (r) =>
              (userUuid: r['user_uuid'] as String, hash: r['hash'] as String),
        )
        .toList();
    _db.execute(
      '''
