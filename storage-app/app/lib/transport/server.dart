...
class PpcServer {
  ...
  Future<void> _handle(HttpRequest req) async {
    try {
      ...
      await _safeJson(req, HttpStatus.requestEntityTooLarge, {
        'error': 'payload_too_large',
        'detail': 'request body exceeds limit',
      });
    } catch (e, s) {
      stderr.writeln('unhandled: $e\n$s');
      await _safeJson(req, HttpStatus.internalServerError, {
        'error': 'internal',
  ...
  Future<void> _route(HttpRequest req) async {
    ...
    // Для PUT блоба лимит = глобальная квота (или 512 MiB по умолчанию).
    final isPut = method == 'PUT' && segs.length == 4 && segs[1] == 'blob';
    final bodyLimit = isPut
        ? (config.maxBytes > 0 ? config.maxBytes : 512 * 1024 * 1024)
        : _maxBodyBytes;
    final body = await _readBody(req, limit: bodyLimit);
    final authHeaders = _extractHeaders(req);
    final auth = await _authenticate(req, body, authHeaders);
    ...
    if (method == 'GET' && _match(segs, ['ppc', 'usage'])) {
      if (!_ownsUser(req.uri.queryParameters['user_id'], authHeaders)) {
        return _forbidden(req);
      }
    ...
    if (segs.length == 4 && segs[0] == 'ppc' && segs[1] == 'blob') {
      ...
      if (!_ownsBlob(userId, hash, authHeaders)) return _forbidden(req);
      switch (method) {
        case 'PUT':
    ...
    if (method == 'GET' &&
        ...
        segs[0] == 'ppc' &&
        segs[1] == 'stat') {
      if (!_ownsBlob(segs[2], segs[3], authHeaders)) return _forbidden(req);
  ...
  Future<void> _handlePair(HttpRequest req) async {
    ...
    final code = j['code'];
    final peerPubkey = j['peer_pubkey'];
    final nodeId = j['node_id'];
    final name = j['name'] ?? '';
    if (code is! String ||
        !RegExp(r'^\d{6}$').hasMatch(code) ||
        peerPubkey is! String ||
        !_validEd25519PublicKey(peerPubkey) ||
        nodeId is! String ||
        nodeId.isEmpty ||
        nodeId.length > 256 ||
      ...
      return _json(req, HttpStatus.badRequest, {
        'error': 'bad_request',
        'detail': 'invalid pairing request',
      });
    }
  ...
  Future<void> _handleUsage(HttpRequest req) async {
    ...
    return _json(req, HttpStatus.ok, Usage(u.bytes, u.files, quota).toJson());
  }
  ...
  Future<void> _handleStat(HttpRequest req, String userId, String hash) async {
    final bool ex;
    try {
  ...
  Future<AuthResult> _authenticate(
    ...
    return verifier.verify(
      ...
      uri: req.uri,
      body: body,
      isPairedIdentity: metaDb.isPairedIdentity,
    );
  }

  bool _ownsUser(String? userId, SignatureHeaders? headers) {
    return headers != null &&
        userId != null &&
        userId == headers.nodeId &&
        userId.length <= 256 &&
        _nodeIdPattern.hasMatch(userId);
  }

  bool _ownsBlob(String userId, String hash, SignatureHeaders? headers) {
    return _ownsUser(userId, headers) && _blobHashPattern.hasMatch(hash);
  }
  ...
  static bool _validEd25519PublicKey(String value) {
  ...
  /// Max body size for non-blob endpoints (pair, revoke, usage).
  static const _maxBodyBytes = 64 * 1024; // 64 KiB

  Future<Uint8List> _readBody(HttpRequest req, {int? limit}) async {
    final maxBytes = limit ?? _maxBodyBytes;
    final chunks = <int>[];
