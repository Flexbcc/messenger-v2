...
class SignatureVerifier {
  ...
  Future<AuthResult> verify({
    ...
    required bool Function(String nodeId, String pubkey) isPairedIdentity,
    int? nowSeconds,
  }) async {
    if (headers == null) return AuthResult.missingHeaders;
    if (headers.nodeId.isEmpty ||
        headers.nodeId.length > 256 ||
        !_nodeIdPattern.hasMatch(headers.nodeId) ||
        headers.pubkey.length > 64 ||
        headers.signatureB64.length > 128) {
    ...
    if (sig.length != 64 || base64.encode(sig) != headers.signatureB64) {
      return AuthResult.badSignature;
    }
    ...
    if (!isPairedIdentity(headers.nodeId, headers.pubkey)) {
      return AuthResult.notPaired;
    }
  ...
  static Uint8List? _decodePubkey(String pubkey) {
    ...
    try {
      ...
      if (b.length != 32 ||
          base64.encode(b) != pubkey.substring(prefix.length)) {
        return null;
      }
      return b;
    } catch (_) {
